locals {
  ...
  s3_buckets = merge(
    ...
    var.options.db_backup_s3 && var.environment.environment == "production" && var.options.preprod_read_write_delete_prod ? { "prod-${var.environment.application_name}-db-backup-bucket-" = {
      bucket_policy_v2 = [
        local.s3_bucket_policies.PreprodReadWriteDeleteBucketPolicy
      ]
      custom_kms_key = var.environment.kms_keys["general"].arn
      iam_policies   = local.requested_s3_iam_policies
    } } : {},
  ...
  s3_bucket_policies = {
    ...
    PreprodReadWriteDeleteBucketPolicy                  = local.iam_policy_statements_s3.S3ReadWriteDeletePreprod[0]
