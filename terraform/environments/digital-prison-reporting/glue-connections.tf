locals {
  nomis_connection_host                 = (local.environment == "development" ? local.nomis_port_forwarded_ip : local.nomis_port)
  ...
  nomis_jdbc_connection_string          = "jdbc:oracle:thin:@${local.nomis_connection_host}:${local.nomis_port}/${local.nomis_service_name}"
}
...
resource "aws_glue_connection" "glue_nomis_connection" {
  ...
  physical_connection_requirements {
    availability_zone      = data.aws_subnet.private_subnets_a.availability_zone
    security_group_id_list = [aws_security_group.glue_nomis_connection_sg.id]
    subnet_id              = data.aws_subnet.private_subnets_a.id
  }
}

resource "aws_security_group" "glue_nomis_connection_sg" {
  #checkov:skip=CKV2_AWS_5

  name        = "${local.project}-nomis-connection_sg"
  description = "Security group to allow glue access to Nomis via JDBC Connection"
  vpc_id      = data.aws_vpc.shared.id
