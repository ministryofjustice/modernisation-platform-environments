...
resource "aws_acm_certificate" "laa_cert" {
  domain_name       = local.is-preproduction || local.is-production ? format("%s-%s.modernisation-platform.service.justice.gov.uk", "laa", local.environment) : format("x.%s-%s.modernisation-platform.service.justice.gov.uk", "laa", local.environment)
