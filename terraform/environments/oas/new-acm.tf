...
locals {
  ...
  domain_types = contains(["development", "test", "preproduction"], local.environment) ? {
  ...
  parent_domain_validation = contains(["development", "test", "preproduction"], local.environment) ? {
    for k, v in local.domain_types : k => v
    if k == "modernisation-platform.service.justice.gov.uk"
  } : {}

  env_domain_validation = contains(["development", "test", "preproduction"], local.environment) ? {
    for k, v in local.domain_types : k => v
    if k != "modernisation-platform.service.justice.gov.uk"
  } : {}
}
...
resource "aws_acm_certificate" "external" {
  count = contains(["development", "test", "preproduction"], local.environment) ? 1 : 0
