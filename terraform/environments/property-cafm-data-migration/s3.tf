...
module "s3_concept_data_uploads_bucket" {
  source             = "github.com/ministryofjustice/modernisation-platform-terraform-s3-bucket?ref=f759060"
  bucket_prefix      = "${local.account_name}-data-concept-uploads-${local.environment_shorthand}-"
  custom_kms_key     = aws_kms_key.shared_kms_key.arn
  versioning_enabled = true
...
module "s3_concept_data_output_bucket" {
  source             = "github.com/ministryofjustice/modernisation-platform-terraform-s3-bucket?ref=f759060"
  bucket_prefix      = "${local.account_name}-data-concept-output-${local.environment_shorthand}-"
  custom_kms_key     = aws_kms_key.shared_kms_key.arn
  versioning_enabled = true
