...
export PGPASSWORD=$NCAS_DB_PASSWORD;
# if database contains schema dbo then store schema name inside variable.
SCHEMA=$(psql -h ${DB_HOSTNAME} -p 5432 -U $NCAS_DB_USERNAME -d $DB_NAME -c "SELECT schema_name FROM information_schema.schemata WHERE schema_name = 'dbo'" | grep -o 'dbo')
echo "Schema = $SCHEMA"
...
if [ "$SCHEMA" == "dbo" ]; then
    ...
    export PGPASSWORD=$NCAS_DB_PASSWORD;
    psql -U $NCAS_DB_USERNAME -h $DB_HOSTNAME -d $DB_NAME -f pre-data.sql
    psql -U $NCAS_DB_USERNAME -h $DB_HOSTNAME -d $DB_NAME -f sequences.sql
    psql -U $NCAS_DB_USERNAME -h $DB_HOSTNAME -d $DB_NAME -f data.sql
    psql -U $NCAS_DB_USERNAME -h $DB_HOSTNAME -d $DB_NAME -f post-data.sql
fi
