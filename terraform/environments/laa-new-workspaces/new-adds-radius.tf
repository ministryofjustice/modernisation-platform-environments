...
resource "aws_directory_service_radius_settings" "workspaces_ad_radius" {
  ...
  # RADIUS server endpoint from workspace-components remote state
  radius_servers = [data.terraform_remote_state.workspace_components.outputs.radius_server_private_ip]
...
output "radius_configuration_complete" {
  ...
  value       = aws_directory_service_radius_settings.workspaces_ad_radius.id != null
}
