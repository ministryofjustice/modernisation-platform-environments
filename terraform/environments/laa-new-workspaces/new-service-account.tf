...
resource "aws_secretsmanager_secret" "lambda_service_account_password" {

  name                    = "${local.application_name}/${local.environment}/lambda-service-account-password"
...
resource "aws_secretsmanager_secret_version" "lambda_service_account_password" {

  secret_id = aws_secretsmanager_secret.lambda_service_account_password.id
  secret_string = jsonencode(
    {
      username = "lambda.workspace"
      password = random_password.lambda_service_account.result
    }
  )
...
resource "terraform_data" "lambda_service_account_password_reset" {
  ...
  depends_on = [
    ...
    aws_secretsmanager_secret_version.lambda_service_account_password,
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      COMMAND_ID=$(aws ssm send-command \
        ...
        --parameters 'commands=["Write-Host \"Setting lambda.workspace password and enabling account...\"","$adminSecret = Get-SECSecretValue -SecretId \"${local.application_name}/${local.environment}/ad-admin-password\"","$adminJson = $adminSecret.SecretString | ConvertFrom-Json","$adminPwd = $adminJson.password","$secureAdminPwd = ConvertTo-SecureString $adminPwd -AsPlainText -Force","$adminCred = New-Object System.Management.Automation.PSCredential(\"${local.application_data.accounts[local.environment].ad_short_name}\\Admin\", $secureAdminPwd)","$serviceSecret = Get-SECSecretValue -SecretId \"${local.application_name}/${local.environment}/lambda-service-account-password\"","$serviceJson = $serviceSecret.SecretString | ConvertFrom-Json","$serviceAcctPwd = $serviceJson.password","$secureServicePwd = ConvertTo-SecureString $serviceAcctPwd -AsPlainText -Force","Set-ADAccountPassword -Identity \"lambda.workspace\" -NewPassword $secureServicePwd -Reset -Credential $adminCred -Server \"${local.application_data.accounts[local.environment].ad_directory_name}\"","Enable-ADAccount -Identity \"lambda.workspace\" -Credential $adminCred -Server \"${local.application_data.accounts[local.environment].ad_directory_name}\"","Write-Host \"Password set and account enabled\"","Get-ADUser -Identity \"lambda.workspace\" -Credential $adminCred -Server \"${local.application_data.accounts[local.environment].ad_directory_name}\" -Properties Enabled | Select-Object Name, SamAccountName, Enabled"]' \
