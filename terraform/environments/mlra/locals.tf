...
locals {
  ...
  task_definition = templatefile("task_definition.json", {
    ...
    db_secret_arn             = "arn:aws:secretsmanager:${local.application_data.accounts[local.environment].region}:${local.env_account_id}:secret:${local.maatdb_password_secret_name}"
    google_tag_manager_id     = "arn:aws:secretsmanager:${local.application_data.accounts[local.environment].region}:${local.env_account_id}:secret:${local.gtm_id_secret_name}"
    infox_client_secret       = "arn:aws:secretsmanager:${local.application_data.accounts[local.environment].region}:${local.env_account_id}:secret:${local.infox_client_secret_name}"
    maat_api_client_id        = "arn:aws:secretsmanager:${local.application_data.accounts[local.environment].region}:${local.env_account_id}:secret:${local.maat_api_client_id_name}"
    maat_api_client_secret    = "arn:aws:secretsmanager:${local.application_data.accounts[local.environment].region}:${local.env_account_id}:secret:${local.maat_api_client_secret_name}"
    app_master_password       = "arn:aws:secretsmanager:${local.application_data.accounts[local.environment].region}:${local.env_account_id}:secret:${local.app_master_password_name}"
    app_salt                  = "arn:aws:secretsmanager:${local.application_data.accounts[local.environment].region}:${local.env_account_id}:secret:${local.app_salt_name}"
    app_derivation_iterations = "arn:aws:secretsmanager:${local.application_data.accounts[local.environment].region}:${local.env_account_id}:secret:${local.app_derivation_iterations_name}"
  })
