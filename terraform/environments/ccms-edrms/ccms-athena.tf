...
resource "aws_athena_named_query" "main_table_edrmsapp" {
  ...
  query = templatefile(
    ...
    {
      bucket     = module.s3-bucket-logging.bucket.id
      key        = local.lb_log_prefix_edrmsapp
...
resource "aws_athena_named_query" "http_requests_ebsapp" {
  name      = lower(format("%s-%s-http-get-requests-edrmsapp", local.application_name, local.environment))
  workgroup = aws_athena_workgroup.lb-access-logs.id
  database  = aws_athena_database.lb-access-logs.name
  query = templatefile(
