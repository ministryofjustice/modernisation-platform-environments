  ...
  "Statement": [
    {
      ...
      "Resource": ["arn:aws:secretsmanager:eu-west-2:*:secret:edrms-secret-*"]
    }
  ]
}
EOF
...
resource "aws_iam_policy" "ecs_parameter_store_policy" {
  name = "${local.application_name}-ecs_parameter_store_policy"
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:ssm:eu-west-2:876083391650:parameter/XXSOA"
    }
  ]
}
EOF
...
resource "aws_iam_role_policy_attachment" "ecs_parameter_store_policy_attachment" {
  role       = aws_iam_role.ecs_task_execution_role.name
