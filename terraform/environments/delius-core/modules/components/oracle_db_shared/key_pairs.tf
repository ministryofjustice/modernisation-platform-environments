...
resource "aws_ssm_parameter" "ec2_user_ssh_key" {
  name        = "/${var.env_name}/oracle_db/ec2-user-ssh-key"
  type        = "SecureString"
  value       = "initial_value_to_be_changed"
  key_id      = var.account_config.kms_keys.general_shared
  description = "ssh private key for ec2-user used for the ${var.env_name} environment"
  tags        = var.tags
...
resource "aws_key_pair" "db_ec2_key_pair" {
  key_name   = "${var.env_name}-oracle-db-ec2-user-key-pair"
