resource "aws_secretsmanager_secret" "delius_core_dba_passwords" {
  name        = join("-",[lookup(var.tags, "environment-name",null),lookup(var.tags, "delius-environment-name",null),"dba-passwords"])
...
resource "aws_secretsmanager_secret" "delius_core_application_passwords" {
  name        = join("-",[lookup(var.tags, "environment-name",null),lookup(var.tags, "delius-environment-name",null),"application-passwords"])
