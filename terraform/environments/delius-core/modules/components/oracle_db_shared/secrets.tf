resource "aws_secretsmanager_secret" "delius_core_dba_passwords" {
  name        = join("-",[lookup(var.tags, "environment-name",null),lookup(var.tags, "delius-environment-name",null),replace(lookup(var.tags, "application",null),"-core",""),"dba-passwords"])
...
resource "aws_secretsmanager_secret" "delius_core_application_passwords" {
  name        =join("-",[lookup(var.tags, "environment-name",null),lookup(var.tags,"delius-environment-name",null),replace(lookup(var.tags, "application",null),"-core",""),"application-passwords"])
