...
resource "aws_secretsmanager_secret" "dms_audit_source_endpoint_db" {
  name        = local.dms_audit_source_endpoint_secret_name
...
data "aws_iam_policy_document" "dms_audit_source_endpoint_db" {
  statement {
    ...
    resources = [aws_secretsmanager_secret.dms_audit_source_endpoint_db.arn]
  }
}

resource "aws_secretsmanager_secret_policy" "dms_audit_source_endpoint_db" {
  secret_arn = aws_secretsmanager_secret.dms_audit_source_endpoint_db.arn
  policy     = data.aws_iam_policy_document.dms_audit_source_endpoint_db.json
}
...
data "aws_iam_policy_document" "dms_user_source_endpoint_db" {
  statement {
    sid    = "DMSRoleToReadTheSecret"
    effect = "Allow"
    principals {
    ...
    actions   = ["secretsmanager:GetSecretValue"]
    resources = [aws_secretsmanager_secret.dms_user_source_endpoint_db.arn]
  }
}

resource "aws_secretsmanager_secret_policy" "dms_user_source_endpoint_db" {
  secret_arn = aws_secretsmanager_secret.dms_user_source_endpoint_db.arn
  policy     = data.aws_iam_policy_document.dms_user_source_endpoint_db.json
}
