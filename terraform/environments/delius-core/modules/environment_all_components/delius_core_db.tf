...
#  name = each.key
    ...
    subnet_id        = var.network_config.private_subnet_a_id
...
resource "aws_security_group" "base_ami_test_instance_sg" {
  name        =  format("%s-%s-base-ami-test-instance-sg", var.env_name, var.db_config.name)
  description = "Controls access to base AMI instance"
  vpc_id      = var.account_info.vpc_id
  tags = merge(local.tags,
    { Name = lower(format("sg-%s-%s-base-ami-test-instance", var.account_info.application_name, var.env_name)) }
  )
}
...
resource "aws_iam_policy" "base_ami_test_instance_iam_assume_policy" {
  ...
  tags = merge(local.tags,
    { Name = "base_ami_test_instance_iam_assume_policy" } #{ Name = lower(format("base_ami_test_instance_iam_assume_policy", var.account_info.application_name, var.env_name)) }
  )
}
...
resource "aws_iam_policy" "business_unit_kms_key_access" {
  ...
  tags = merge(local.tags,
    { Name = "business_unit_kms_key_access_policy" } #{ Name = lower(format("business_unit_kms_key_access_policy", var.account_info.application_name, var.env_name)) }
  )
}
...
resource "aws_iam_policy" "core_shared_services_bucket_access" {
  ...
  tags = merge(local.tags,
    { Name = "core_shared_services_bucket_access_policy" } #{ Name = lower(format("core_shared_services_bucket_access_policy", var.account_info.application_name, var.env_name)) }
  )
}
...
resource "aws_iam_policy" "ec2_access_for_ansible" {
  ...
  tags = merge(local.tags,
    { Name = "ec2_access_for_ansible_policy" } #{ Name = lower(format("ec2_access_for_ansible_policy", var.account_info.application_name, var.env_name)) }
  )
}
