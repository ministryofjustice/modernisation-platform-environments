...
resource "aws_instance" "tariff_app" {
  ...
  user_data                   = <<EOF
            ...
            EOF
  # vpc_security_group_ids      = local.environment == "production" ? [module.tariff_app_prod_security_group[0].security_group_id, aws_security_group.tariff_app_prod_security_group[0].id] : [aws_security_group.tariff_app_security_group[0].id]
