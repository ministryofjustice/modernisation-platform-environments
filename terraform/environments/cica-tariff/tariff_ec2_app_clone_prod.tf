...
resource "aws_security_group_rule" "temp_prod_ssm_only_egress" {
  count             = local.environment == "production" ? 1 : 0
  security_group_id = aws_security_group.temp_prod_ssm_only[0].id
...
resource "aws_instance" "tariff_app2_prod_clone" {
  ...
  tags = merge(tomap({
    "Name"     = lower(format("ec2-%s-%s-app2-clone", local.application_name, local.environment)),
    "hostname" = "${local.application_name}-app2-poc-clone",
    }), local.tags, local.environment != "test" ? tomap({ "backup" = "true" }) : tomap({})
  )
}
# Temporary SG to restrict access to/from Clone above during configuration phase
