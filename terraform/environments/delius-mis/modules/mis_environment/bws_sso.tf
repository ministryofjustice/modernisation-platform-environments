module "bws_sso_instance" {
  ...
  name = "${var.app_name}-${var.env_name}-bws-sso-${count.index + 1}"

  ami_name  = var.bws_sso_config.ami_name
  ami_owner = var.bws_sso_config.ami_owner
  instance = merge(var.bws_sso_config.instance_config, {
  ...
  ebs_volumes                   = var.bws_sso_config.ebs_volumes
  ebs_volume_config             = var.bws_sso_config.ebs_volumes_config
  ebs_volume_tags               = local.tags
  route53_records = {
    create_internal_record = false
    create_external_record = false
  }
  iam_resource_names_prefix = "${var.env_name}-bws_sso-${count.index + 1}"
  ...
  user_data_cloud_init = {
    args = {
      branch       = var.bws_sso_config.ansible_branch
