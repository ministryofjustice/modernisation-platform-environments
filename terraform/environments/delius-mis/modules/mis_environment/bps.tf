resource "aws_security_group" "bps_ec2" {
  #checkov:skip=CKV2_AWS_5 "ignore"
  name        = "${var.app_name}-${var.env_name}-bps-ec2-instance-sg"
  description = "Security group for BPS EC2"
...
resource "aws_vpc_security_group_ingress_rule" "bps_ec2" {
  ...
  security_group_id = resource.aws_security_group.bps_ec2.id
...
resource "aws_vpc_security_group_egress_rule" "bps_ec2" {
  for_each = {
    ...
    ldap-tcp-to-vpc   = { ip_protocol = "TCP", port = 389, cidr_ipv4 = module.ip_addresses.mp_cidr[local.vpc_name] }
    ldap-udp-to-vpc   = { ip_protocol = "UDP", port = 389, cidr_ipv4 = module.ip_addresses.mp_cidr[local.vpc_name] }
