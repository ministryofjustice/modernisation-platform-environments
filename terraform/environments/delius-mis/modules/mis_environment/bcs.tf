...
resource "aws_vpc_security_group_ingress_rule" "bcs_ec2" {
  for_each = {
    all-from-self = { referenced_security_group_id = aws_security_group.bcs_ec2.id }
...
resource "aws_vpc_security_group_egress_rule" "bcs_ec2" {
  for_each = {
    all-to-self          = { referenced_security_group_id = aws_security_group.bcs_ec2.id }
...
module "bcs_instance" {
  ...
  instance = merge(var.bcs_config.instance_config, {
...
module "bcs_win_instance" {
  ...
  name = "${var.app_name}-${var.env_name}-bcs-win-${count.index + 1}"

  ami_name  = var.bcs_config_win.ami_name
  ami_owner = "self"
  instance = merge(var.bcs_config_win.instance_config, {
    vpc_security_group_ids = [
      aws_security_group.legacy.id,
      aws_security_group.bcs_ec2.id,
  ...
  ebs_kms_key_id                = var.account_config.kms_keys["ebs_shared"]
  ebs_volumes_copy_all_from_ami = false
  ebs_volumes                   = var.bcs_config_win.ebs_volumes
  ebs_volume_config             = var.bcs_config_win.ebs_volumes_config
  ebs_volume_tags               = local.tags
  route53_records = {
    create_internal_record = false
    create_external_record = false
  }
  iam_resource_names_prefix = "${var.env_name}-bcs-win-${count.index + 1}"
  instance_profile_policies = [
    # "arn:aws:iam::aws:policy/AmazonSSMManagedInstanceCore", added by module
    aws_iam_policy.secrets_manager.arn,
    aws_iam_policy.ec2_automation.arn
  ...
  user_data = templatefile(
    "${path.module}/templates/user-data-pwsh.yaml.tftpl", {
      branch = var.bcs_config_win.powershell_branch
    }
  )
  ...
  tags = merge(
    local.tags,
    {
      computer-name = "${var.bcs_config_win.computer_name}-${count.index + 1}"
      domain-name   = var.environment_config.ad_domain_name
      server-type   = "DeliusMisBcs"
    }
  )

  # cloudwatch_metric_alarms = var.bcs_config_win.cloudwatch_metric_alarms != null ? var.bcs_config_win.cloudwatch_metric_alarms : merge(
  #   local.cloudwatch_metric_alarms.ec2
  # )
}
