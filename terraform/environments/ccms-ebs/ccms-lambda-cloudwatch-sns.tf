...
resource "aws_lambda_function" "cloudwatch_sns" {
  ...
  source_code_hash = filebase64sha256(join("", [for f in fileset("${path.module}/lambda/cloudwatch_alarm_slack_integration", "**") : file("${path.module}/lambda/cloudwatch_alarm_slack_integration/${f}")]))
