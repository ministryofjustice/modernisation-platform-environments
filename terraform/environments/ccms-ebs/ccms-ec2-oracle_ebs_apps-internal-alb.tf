resource "aws_lb" "ebsapps_internal_alb" {
  name               = lower(format("lb-%s-ebsapp-internal", local.application_name))
  internal           = true
  load_balancer_type = "application"
  security_groups    = [aws_security_group.sg_ebsapps_internal_alb.id]
  ...
  access_logs {
    ...
    prefix  = local.lb_log_prefix_ebsapp_internal
    enabled = true
  }
...
resource "aws_lb_target_group" "ebsapp_internal_tg" {
...
resource "aws_lb_target_group_attachment" "ebsapps_internal" {
  ...
  target_group_arn = aws_lb_target_group.ebsapp_internal_tg.arn
