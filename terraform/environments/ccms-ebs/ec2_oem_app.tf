data "template_file" "oem-shell-script-app" {
...
data "template_cloudinit_config" "cloudinit-oem-app" {
  gzip          = false
  base64_encode = false

  part {
    content_type = "text/x-shellscript"
    content      = data.template_file.oem-shell-script-app.rendered
  }
}
...
resource "aws_instance" "oem_app" {
  ...
  user_data                   = data.template_cloudinit_config.cloudinit-oem-app.rendered
...
resource "aws_security_group" "oem_app_security_group_1" {
  name_prefix = "${local.application_name_oem}-app-server-sg-1-"
  description = "Access to the ebs app server"
  vpc_id      = data.aws_vpc.shared.id

  tags = merge(tomap(
    { "Name" = "${local.application_name_oem}-app-server-sg-1" }
  ), local.tags)
...
resource "aws_security_group" "oem_app_security_group_2" {
  name_prefix = "${local.application_name_oem}-app-server-sg-2-"
  description = "Access to the ebs app server"
  vpc_id      = data.aws_vpc.shared.id

  tags = merge(tomap(
    { "Name" = "${local.application_name_oem}-app-server-sg-2" }
  ), local.tags)
