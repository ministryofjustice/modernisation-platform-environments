...
def lambda_handler(event, context):
    mode = event.get("mode", "ALLOW").upper()
    web_acl_name = os.environ["WEB_ACL_NAME"]
    web_acl_id = os.environ["WEB_ACL_ID"]
    ...
    # Find and update only the target rule's Action
    rules = []
    found = False
            ...
            if mode == "BLOCK":
                updated_rule["Action"] = {
                    "Block": {
                        "CustomResponse": {
                            "ResponseCode": 503,
                            "CustomResponseBodyKey": "maintenance_html"  # must match Terraform
                        }
                    }
                }
            else:  # ALLOW
                updated_rule["Action"] = {"Allow": {}}
            rules.append(updated_rule)
        else:
            rules.append(rule)  # preserve other rules

    if not found:
        raise RuntimeError(f"Rule '{target_rule_name}' not found in WebACL")

    # Update WebACL — DO NOT include CustomResponseBodies
    waf.update_web_acl(
        Name=web_acl_name,
        Scope=scope,
        Id=web_acl_id,
