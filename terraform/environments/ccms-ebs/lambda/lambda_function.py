...
def lambda_handler(event: Dict[str, Any], context: Optional[Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Combine all log events into a single message and check for errors
        combined_message, is_error = process_all_events(log_data.get("logEvents", []))
        
        if combined_message:
            # Generate dynamic title based on success/error status
...
def process_all_events(log_events: List[Dict[str, Any]]) -> Tuple[Optional[str], bool]:
    ...
    Returns:
        Tuple of (combined message as a single string, is_error flag)
    """
    if not log_events:
    ...
    for log_event in log_events:
        try:
            ...
            # Check if message is empty and continue if so
            if not message:
            ...
            # Check log level from message to determine if it's an error
            log_level = next(
    ...
    # Combine all message parts into a single consolidated message with newlines
    consolidated_message = ""
    for part in message_parts:
        if consolidated_message:
            consolidated_message += "\n"
        consolidated_message += part

    # Add separator between events if multiple
    if len(message_parts) > 1:
        separator = "\n" + "=" * 50 + "\n"
        combined_message = separator + separator.join(message_parts)
    else:
        combined_message = consolidated_message
    ...
    return (combined_message if message_parts else None, is_error)
def process_all_events(log_events: List[Dict[str, Any]]) -> Tuple[Optional[str], bool]:
    """
    Process all log events and combine them into a single consolidated message.
    
    Args:
        log_events: List of log events from CloudWatch Logs
        
    Returns:
        Tuple of (consolidated message as a single string, is_error flag)
    """
    if not log_events:
        logger.warning("No log events provided")
        return None, False
    ...
    for log_event in log_events:
        try:
            ...
            # Skip events with no message content
            ...
            # Add message to consolidated message with newline
