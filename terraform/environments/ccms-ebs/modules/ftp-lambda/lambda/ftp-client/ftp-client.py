"""
AWS Lambda function for FTP/FTPS/SFTP file transfers between S3 and remote servers.
Supports multiple protocols, authentication methods, and file filtering.
"""
...
class CurlOptionsLogger:
    ...
    def get_curl_command(self, base_url="[URL]"):
        ...
        # Map pycurl options to curl command line flags
        option_map = {
            pycurl.TIMEOUT: lambda v: f"--max-time {v}",
            pycurl.SSL_VERIFYPEER: lambda v: "-k" if v == 0 else "",
            pycurl.SSL_VERIFYHOST: lambda v: "" if v == 0 else "",  # Covered by SSL_VERIFYPEER
            pycurl.USE_SSL: lambda v: "--ssl-reqd" if v == pycurl.USESSL_ALL else "",
            pycurl.FTPSSLAUTH: lambda v: "--ftp-ssl" if v == pycurl.FTPAUTH_DEFAULT else "",
            pycurl.VERBOSE: lambda v: "-v" if v == 1 else "",
            pycurl.CAINFO: lambda v: f"--cacert [CERT_FILE]",
            pycurl.SSLCERT: lambda v: f"--cert [CLIENT_CERT]",
            pycurl.SSLKEY: lambda v: f"--key [PRIVATE_KEY]",
            pycurl.USERPWD: lambda v: f"-u [USER:PASS]",
            pycurl.SSH_PRIVATE_KEYFILE: lambda v: f"--key [SSH_KEY]",
            pycurl.UPLOAD: lambda v: "-T [FILE]" if v == 1 else "",
            pycurl.POST: lambda v: "-X POST" if v == 1 else "",
            pycurl.DIRLISTONLY: lambda v: "" if v == 1 else "",  # FTP specific, no direct curl equivalent
        }
        ...
        for option, value, description in self.options:
            if option == pycurl.URL:
                url = str(value) if not any(x in str(value) for x in ["password", "user"]) else "[URL_WITH_CREDENTIALS]"
...
class ConfigValidator:
    ...
    def validate_mandatory_fields(config_dict: Dict[str, Any], field_name: str) -> None:
        ...
        # Password is mandatory for FTP/FTPS, optional for SFTP (if SSH key provided)
        protocol = config_dict.get("protocol")
        password = config_dict.get("password")
        ssh_key = config_dict.get("ssh_key")
        
        if protocol in (TransferProtocol.FTP, TransferProtocol.FTPS):
            # FTP/FTPS always requires password
            mandatory_fields["password"] = password
        elif protocol == TransferProtocol.SFTP:
            # SFTP requires either password or SSH key
            if not password and not ssh_key:
                raise ValueError("SFTP requires either password or SSH key for authentication")
    ...
    def validate_transfer_type(transfer_type_str: Optional[str]) -> "TransferType":
        ...
        transfer_type_map = {
            "FTP_UPLOAD": TransferType.FTP_UPLOAD,
            "FTP_DOWNLOAD": TransferType.FTP_DOWNLOAD,
            "SFTP_UPLOAD": TransferType.SFTP_UPLOAD,
            "SFTP_DOWNLOAD": TransferType.SFTP_DOWNLOAD,
        }
        transfer_type = transfer_type_map.get(transfer_type_str)
        if not transfer_type:
            raise ValueError(f"Invalid transfer type: {transfer_type_str}")
        return transfer_type

    @staticmethod
    def validate_protocol(protocol_str: Optional[str]) -> "TransferProtocol":
        """Validate and convert protocol string."""
        if not protocol_str:
            raise ValueError("PROTOCOL environment variable is required")
...
class TransferConfig:
    """Configuration for file transfers with validation."""
    ...
    def __post_init__(self):
        ...
        logger.info(
            f"Configuration validated for {self.transfer_type.name} using {self.protocol.value}"
        )
...
class NotificationService:
    ...
    def __init__(self, webhook_url: str, function_name: str = "FTP Transfer Lambda"):
        if not webhook_url:
...
class FileTransferProtocol(ABC):
    """Abstract base class defining the interface for file transfer implementations."""

    @abstractmethod
    def download_files(self, config: TransferConfig, s3_client: S3Client) -> List[str]:
        """Download files from remote server to S3."""
        pass

    @abstractmethod
    def upload_files(self, config: TransferConfig, s3_client: S3Client) -> List[str]:
        """Upload files from S3 to remote server."""
        pass


class PycurlTransferHandler(FileTransferProtocol):
    """Handler for FTP/FTPS transfers using pycurl."""
    ...
    def _create_curl_instance(self, config: TransferConfig) -> pycurl.Curl:
        ...
        set_curl_option(curl, pycurl.TIMEOUT, 600, "Set connection timeout", curl_logger)

        # SSL/TLS configuration
        if config.skip_key_verification:
            set_curl_option(curl, pycurl.SSL_VERIFYPEER, 0, "Disable SSL peer verification", curl_logger)
            set_curl_option(curl, pycurl.SSL_VERIFYHOST, 0, "Disable SSL host verification", curl_logger)
            logger.warning("SSL verification disabled - connection is insecure")

        if config.protocol == TransferProtocol.FTPS or config.require_ssl:
            set_curl_option(curl, pycurl.USE_SSL, pycurl.USESSL_ALL, "Enable SSL/TLS for all operations", curl_logger)
            set_curl_option(curl, pycurl.FTPSSLAUTH, pycurl.FTPAUTH_DEFAULT, "Set SSL auth method", curl_logger)
        ...
        if config.ca_cert:
            with tempfile.NamedTemporaryFile(
                ...
                ca_file.write(base64.b64decode(config.ca_cert))
                ca_file_path = ca_file.name
            curl.setopt(pycurl.CAINFO, ca_file_path)
            logger.info(f"Using CA certificate: {ca_file_path}")
        ...
        if config.cert:
            with tempfile.NamedTemporaryFile(
                ...
                cert_file.write(base64.b64decode(config.cert))
                cert_file_path = cert_file.name
            curl.setopt(pycurl.SSLCERT, cert_file_path)
            logger.info(f"Using client certificate: {cert_file_path}")

        if config.key:
            curl.setopt(pycurl.SSLKEY, config.key)
            if config.key_type:
                curl.setopt(pycurl.SSLKEYTYPE, config.key_type)

        # Authentication
        if config.password:
            set_curl_option(curl, pycurl.USERPWD, f"{config.user}:{config.password}", "Set user credentials", curl_logger)
            ...
            set_curl_option(curl, pycurl.USERPWD, f"{config.user}:", "Set username for SSH key auth", curl_logger)
    ...
    def _build_url(self, config: TransferConfig, filename: str = "") -> str:
        ...
        protocol = (
            "ftp"
            if config.protocol in (TransferProtocol.FTP, TransferProtocol.FTPS)
            else "sftp"
        )
        ...
        url = f"{protocol}://{config.host}{port_part}{remote_path}{filename}"
    ...
    def list_remote_files(self, config: TransferConfig) -> List[str]:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Failed to decode FTP directory listing: {e}")
            ...
            files = [
                f for f in files 
                if f and f not in (".", "..") and not f.endswith("/")
            ]
    ...
    def delete_remote_file(self, config: TransferConfig, remote_file: str) -> None:
        ...
        try:
            curl.setopt(pycurl.URL, url)
            curl.setopt(pycurl.QUOTE, [f"DELE {remote_file}"])

            logger.info(f"Deleting remote file: {remote_file}")
            curl.perform()

            logger.info(f"Successfully deleted: {remote_file}")

        except pycurl.error as e:
            # Some FTP servers return specific error codes for delete operations
            if "RETR response: 550" not in str(e):
                logger.warning(f"Delete operation may have failed: {e}")
        except Exception as e:
    ...
    def download_files(self, config: TransferConfig, s3_client: S3Client) -> List[str]:
        ...
        try:
            ...
            if not remote_files:
                ...
                #     f"No files found on {config.host}{config.remote_path}",
                # )
            ...
            if fail_count > 0:
                ...
                failed_files_list = "\n• " + "\n• ".join(
                    f"{filename}: {error_msg}" for filename, error_msg in failed_files_display
                )
    ...
    def upload_files(self, config: TransferConfig, s3_client: S3Client) -> List[str]:
        ...
        try:
            ...
            for obj in contents:
                ...
                obj_key = obj.get("Key", "")
                if obj_key and obj_key != config.local_path and not obj_key.endswith("/"):
            ...
            if not s3_objects:
                ...
                # self.notification_service.send_notification(
                #     "No Files to Upload",
                #     f"No files found in `s3://{config.s3_bucket}/{config.local_path}`",
                # )
            ...
            if success_count > 0:
                ...
                uploaded_filenames = [os.path.basename(s3_key) for s3_key in uploaded_files]
            ...
            if fail_count > 0:
                ...
                failed_files_display = failed_files[:5]  # Limit to first 5 files
                failed_files_list = "\n• " + "\n• ".join(
                    f"{os.path.basename(s3_key)}: {error_msg}" for s3_key, error_msg in failed_files_display
                )
                ellipsis = "\n• ..." if fail_count > 5 else ""
            ...
            self.notification_service.send_notification(
                "FTP Upload Failed", f"Upload operation failed:\n{str(e)}", is_error=True
            )
            raise
...
class SftpPycurlHandler(PycurlTransferHandler):
    """Handler for SFTP transfers using pycurl."""

    def _create_curl_instance(self, config: TransferConfig) -> pycurl.Curl:
        """Create and configure a pycurl instance for SFTP."""
        # Get the base curl instance (which already has logging)
        curl = super()._create_curl_instance(config)
        
        # Create a new logger for SFTP-specific options
        sftp_logger = CurlOptionsLogger()
        ...
        if config.ssh_key:
            ...
            set_curl_option(curl, pycurl.SSH_PRIVATE_KEYFILE, key_file_path, "Set SSH private key file", sftp_logger)
            logger.info("Using SSH private key for authentication")

        # Set SSH host key checking
        if config.skip_key_verification:
            # Disable SSH host key verification for testing
            # Note: This should only be used in development/testing environments
            
            # The key is to NOT set any SSH host key options at all
            # This allows the connection without fingerprint verification
            
            # Disable SSL peer verification for FTPS over SFTP
            set_curl_option(curl, pycurl.SSL_VERIFYPEER, 0, "Disable SSL peer verification for SFTP", sftp_logger)
            set_curl_option(curl, pycurl.SSL_VERIFYHOST, 0, "Disable SSL host verification for SFTP", sftp_logger)
            
            # Disable strict host key checking by not providing known hosts
            # This is the cleanest way to bypass SSH host key verification
            logger.warning("SSH host key verification disabled - connection is insecure")
        else:
            # For production environments, you should provide the expected host key fingerprint
            # This can be added as a future enhancement to accept host key via configuration
            logger.info("SSH host key verification enabled")

        # Log SFTP-specific options if any were added
        if sftp_logger.options:
            sftp_logger.log_final_command("[SFTP_URL]")
...
class TransferFactory:
    """Factory for creating appropriate transfer handlers."""

    @staticmethod
    def create_handler(
        config: TransferConfig, notification_service: NotificationService
    ) -> FileTransferProtocol:
        """Create the appropriate transfer handler based on protocol."""
        if config.protocol in (TransferProtocol.FTP, TransferProtocol.FTPS):
            return PycurlTransferHandler(notification_service)
        elif config.protocol == TransferProtocol.SFTP:
            return SftpPycurlHandler(notification_service)
        else:
...
def parse_config_from_env_and_secrets(
    ...
    # Validate transfer type and protocol using validator
    transfer_type = ConfigValidator.validate_transfer_type(env_data.get("TRANSFERTYPE"))
    protocol = ConfigValidator.validate_protocol(env_data.get("PROTOCOL"))
    ...
    config = TransferConfig(
        ...
        slack_webhook=ConfigValidator.get_mandatory_env(env_data, "SLACK_WEBHOOK"),
        # Optional settings from various sources
        port=port,
        ssh_key=ssh_key,  # From secrets if present
        require_ssl=str(env_data.get("REQUIRE_SSL", "")).upper() == "YES",
        skip_key_verification=str(env_data.get("SKIP_KEY_VERIFICATION", "")).upper() == "YES",
        ca_cert=env_data.get("CA_CERT"),  # Base64 encoded in env
        cert=env_data.get("CERT"),  # Base64 encoded in env
        key=env_data.get("KEY"),
        key_type=env_data.get("KEY_TYPE"),
...
def lambda_handler(
    ...
    This function orchestrates the FTP/SFTP file transfer process by:
    1. Loading configuration from both environment variables and AWS Secrets Manager
    2. Initializing the appropriate transfer handler
    ...
    """
    logger.info("Starting FTP Transfer Lambda execution")
    ...
    try:
        ...
        env_config = {
            ...
            "PROTOCOL": get_env_variable("PROTOCOL", required=True),
            ...
            "SLACK_WEBHOOK": get_env_variable("SLACK_WEBHOOK", required=True),
            # Optional environment variables
            "PORT": get_env_variable("PORT", required=False),
            "REQUIRE_SSL": get_env_variable("REQUIRE_SSL", required=False),
            "SKIP_KEY_VERIFICATION": get_env_variable("SKIP_KEY_VERIFICATION", required=False),
            "CA_CERT": get_env_variable("CA_CERT", required=False),
            "CERT": get_env_variable("CERT", required=False),
            "KEY": get_env_variable("KEY", required=False),
            "KEY_TYPE": get_env_variable("KEY_TYPE", required=False),
        ...
        # Always require USER and HOST
        required_secrets = ["USER", "HOST"]
        ...
        if not has_password and not has_ssh_key:
            raise ValueError("Missing authentication credentials: either PASSWORD or SSH_KEY must be provided")
        ...
        # Initialize services
        notification_service = NotificationService(config.slack_webhook, context.function_name)
        s3_client = cast(S3Client, boto3.client("s3"))

        # Create transfer handler
        handler = TransferFactory.create_handler(config, notification_service)

        # Execute transfer based on type
        if config.transfer_type in (
            TransferType.FTP_DOWNLOAD,
...
if __name__ == "__main__":
    # Example test event
    test_event = {"secret_name": "ftp-transfer-credentials"}

    # Mock context
    class MockContext:
        function_name = "ftp-transfer-lambda"
