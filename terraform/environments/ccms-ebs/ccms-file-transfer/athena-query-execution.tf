resource "null_resource" "execute_create_table_queries" {
  triggers = {
    query_ids = join(",", ["${aws_athena_named_query.main_table_sftpinternal_query.id}", "${aws_athena_named_query.http_requests_sftpinternal_query.id}"
    ])
  }
...
aws athena start-query-execution \
  --query-string "$(aws athena get-named-query --named-query-id ${aws_athena_named_query.main_table_sftpinternal_query.id} --query 'NamedQuery.QueryString' --output text)" \
...
aws athena start-query-execution \
  --query-string "$(aws athena get-named-query --named-query-id ${aws_athena_named_query.http_requests_sftpinternal_query.id} --query 'NamedQuery.QueryString' --output text)" \
  --work-group ${aws_athena_workgroup.lb-access-logs.name} \
  --query-execution-context Database=${aws_athena_database.lb-access-logs.name} \
  --region ${data.aws_region.current.name}
EOF
