data "template_file" "launch-template" {
  ...
  vars = {
    cluster_name       = "${local.sftp_suffix}-cluster"
...
resource "aws_launch_template" "ec2-launch-template" {
  name_prefix   = local.sftp_env_suffix
  ...
  tag_specifications {
    ...
    tags = merge(local.tags,
      { Name = "${local.sftp_env_suffix}-cluster" }
    )
  }
  ...
  tags = merge(local.tags,
    { Name = "${local.sftp_env_suffix}-cluster-template" }
  )

}

resource "aws_autoscaling_group" "cluster-scaling-group" {
  name                = "${local.sftp_env_suffix}-auto-scaling-group"
