...
# variable "scope_ssogen" {
#   default = "REGIONAL"
# }

# variable "rule_name_ssogen" {
#   default = "ssogen-trusted-rule-ip-set"
# }

# data "archive_file" "ssogen_waf_toggle_zip" {
#   type        = "zip"
...
#   name  = "ssogen_internal_waf"
#   scope = "REGIONAL"
# }


# #Create IAM Role and Policy for Lambda
# resource "aws_iam_role" "ssogen_waf_lambda_role" {
#   name = "waf-toggle-role-ssogen-${local.environment}"
...
# resource "aws_iam_role_policy" "ssogen_waf_lambda_policy" {
#   name = "waf-toggle-policy-ssogen-${local.environment}"
#   role = aws_iam_role.ssogen_waf_lambda_role.id
#   policy = jsonencode({
#     Version = "2012-10-17",
#     Statement = [
#       { Effect = "Allow",
...
# resource "aws_lambda_function" "ssogen_waf_toggle" {
#   function_name    = "waf-toggle-${local.environment}"
#   role             = aws_iam_role.ssogen_waf_lambda_role.arn
#   filename         = data.archive_file.ssogen_waf_toggle_zip.output_path
#   source_code_hash = data.archive_file.ssogen_waf_toggle_zip.output_base64sha256
#   handler          = "waf_lambda_function.lambda_handler"
#   runtime          = "python3.11"
#   timeout          = 30
#   environment {
#     variables = {
#       SCOPE        = var.scope_ssogen
#       WEB_ACL_NAME = data.aws_wafv2_web_acl.ssogen_waf_web_acl.name
#       WEB_ACL_ID   = data.aws_wafv2_web_acl.ssogen_waf_web_acl.id
#       RULE_NAME    = var.rule_name_ssogen
