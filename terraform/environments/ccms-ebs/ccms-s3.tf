...
module "ccms_ebs_shared" { #tfsec:ignore:aws-s3-enable-versioning
  # v8.2.0 = https://github.com/ministryofjustice/modernisation-platform-terraform-s3-bucket/commit/52a40b0dd18aaef0d7c5565d93cc8997aad79636
  source = "github.com/ministryofjustice/modernisation-platform-terraform-s3-bucket?ref=52a40b0dd18aaef0d7c5565d93cc8997aad79636"

  bucket_name = "${local.application_name}-${local.environment}-ccms_ebs_shared"
  ...
  log_bucket = local.logging_bucket_name
  log_prefix = "s3access/${local.application_name}-${local.environment}-ccms_ebs_shared"
...
resource "aws_s3_object" "folder" {
  bucket = module.ccms_ebs_shared.bucket.id
  for_each = {
...
module "lambda_payment_load" { #tfsec:ignore:aws-s3-enable-versioning
  # v8.2.0 = https://github.com/ministryofjustice/modernisation-platform-terraform-s3-bucket/commit/52a40b0dd18aaef0d7c5565d93cc8997aad79636
  source = "github.com/ministryofjustice/modernisation-platform-terraform-s3-bucket?ref=52a40b0dd18aaef0d7c5565d93cc8997aad79636"

  bucket_name = "${local.application_name}-${local.environment}-lambda_payment_load"
  sse_algorithm      = "AES256"
  custom_kms_key     = ""
  #  bucket_prefix      = "s3-bucket-example"
  versioning_enabled = true
  ...
  log_bucket = local.logging_bucket_name
  log_prefix = "s3access/${local.application_name}-${local.environment}-lambda_payment_load"
