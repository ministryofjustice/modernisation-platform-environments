...
resource "aws_route53_record" "external_validation" {
  ...
  for_each = {
    for dvo in local.cert_opts : dvo.domain_name => {
      ...
      ) ? aws.core-network-services : aws.core-vpc
    }
  }
...
# resource "aws_acm_certificate_validation" "external_core_vpc" {
#   count = local.is-production ? 1 : 1
...
#   certificate_arn         = local.cert_arn
#   validation_record_fqdns = [for record in aws_route53_record.external_validation_core_vpc : record.fqdn]
