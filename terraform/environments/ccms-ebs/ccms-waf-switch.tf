...
resource "aws_lambda_function" "waf_toggle" {
  ...
  environment {
    variables = {
      ...
      NORMAL_WEB_ACL_ARN     = aws_wafv2_web_acl.ebs_web_acl.arn
      MAINTENANCE_WEB_ACL_ARN = aws_wafv2_web_acl.waf_web_acl_maintenance.arn
...
resource "aws_cloudwatch_event_rule" "waf_allow_0700_uk" {
  ...
  schedule_expression = "cron(40 00 ? * MON-SUN *)"
