resource "aws_instance" "ec2_accessgate-test" {
  ...
  key_name                    = local.application_data.accounts[local.environment].key_name2
...
EOF
  ...
  tags = merge(local.tags,
    { Name = lower(format("ec2-%s-%s-accessgate-testing-%s", local.application_name, local.environment, count.index + 1)) }
  )
