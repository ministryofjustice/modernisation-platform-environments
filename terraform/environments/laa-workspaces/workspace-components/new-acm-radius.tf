...
locals {
  ...
  radius_domain_types = local.environment == "development" ? {
    for dvo in aws_acm_certificate.radius_portal[0].domain_validation_options : dvo.domain_name => {
  ...
  radius_parent_domain_validation = local.environment == "development" ? {
    for k, v in local.radius_domain_types : k => v
    if k == "modernisation-platform.service.justice.gov.uk"
  } : {}

  radius_env_domain_validation = local.environment == "development" ? {
    for k, v in local.radius_domain_types : k => v
    if k != "modernisation-platform.service.justice.gov.uk"
  } : {}
}
