...
locals {
  ...
  drone_incursion_alert_emails = [
    "stuart.logan@justice.gov.uk",
    "vishal.shah@justice.gov.uk",
  ]
  
  #drone_incursion_alert_emails = compact(split(",", try(aws_ssm_parameter.drone_incursion_alert_emails[0].value, "test@test.com,test2@test.com")))
  # TODO: uncomment if a sender ID is created and sms is out of sandbox mode.
