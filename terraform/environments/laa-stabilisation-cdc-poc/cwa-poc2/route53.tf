resource "aws_route53_record" "database" {
  ...
  name     = "${local.database_hostname}.${var.route53_zone_external}"
  type     = "A"
  ttl      = 900
...
resource "aws_route53_record" "concurrent_manager" {
  provider = aws.core-vpc
  zone_id  = var.route53_zone_external_id
  name     = "${local.cm_hostname}.${var.route53_zone_external}"
  type     = "A"
  ttl      = 900
...
resource "aws_route53_record" "app1" {
  provider = aws.core-vpc
  zone_id  = var.route53_zone_external_id
  name     = "${local.appserver1_hostname}.${var.route53_zone_external}"
  type     = "A"
  ttl      = 900
