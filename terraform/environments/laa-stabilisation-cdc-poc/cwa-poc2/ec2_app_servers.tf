...
echo "$PRIVATE_IP	${local.application_name_short}-app1.${var.route53_zone_external}		${local.appserver1_hostname}" >> /etc/hosts
...
sed -i 's/aws.${local.application_data.accounts[local.environment].old_domain_name}/${var.route53_zone_external}/g' /CWA/app/appl/admin/CWA_cwa-app1.xml
sed -i 's/${local.application_data.accounts[local.environment].old_domain_name}/${var.route53_zone_external}/g' /CWA/app/appl/admin/CWA_cwa-app1.xml
...
resource "aws_instance" "app1" {
  ami                         = local.application_data.accounts[local.environment].cwa_poc2_app_ami_id
  availability_zone           = "eu-west-2a"
  instance_type               = local.application_data.accounts[local.environment].cwa_poc2_app_instance_type
  monitoring                  = true
