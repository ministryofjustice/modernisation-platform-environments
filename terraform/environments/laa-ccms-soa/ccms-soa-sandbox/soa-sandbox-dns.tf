...
resource "aws_route53_record" "admin" {
  ...
  name     = local.application_data.accounts[local.environment][local.component_name].admin_hostname
  type     = "A"
  alias {
...
resource "aws_route53_record" "managed" {
  provider = aws.core-vpc
  zone_id  = data.aws_route53_zone.external.zone_id
  name     = local.application_data.accounts[local.environment][local.component_name].managed_hostname
  type     = "A"
  alias {
