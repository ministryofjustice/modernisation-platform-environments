#--Admin
resource "aws_route53_record" "admin-sandbox" {
  provider = aws.core-vpc
  zone_id  = data.aws_route53_zone.external.zone_id
  name     = local.component_name.admin_hostname
  type     = "A"
  alias {
...
#--Managed
resource "aws_route53_record" "managed-sandbox" {
  provider = aws.core-vpc
  zone_id  = data.aws_route53_zone.external.zone_id
  name     = local.component_name.managed_hostname
  type     = "A"
  alias {
