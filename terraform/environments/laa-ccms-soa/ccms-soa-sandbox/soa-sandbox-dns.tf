...
resource "aws_route53_record" "admin" {
  ...
  name     = "${local.component_name}.${local.application_data.accounts[local.environment].admin_domain}"
  type     = "A"
  alias {
...
resource "aws_route53_record" "managed" {
  provider = aws.core-vpc
  zone_id  = data.aws_route53_zone.external.zone_id
  name     = "${local.component_name}.${local.application_data.accounts[local.environment].managed_domain}"
  type     = "A"
  alias {
