resource "aws_db_subnet_group" "soa_sandbox" {
  name_prefix = "main-soa-sandbox"
...
resource "aws_db_option_group" "soa_oracle_19" {
  name_prefix          = "soasandbox-soa-db-option-group"
...
resource "aws_db_instance" "soa_db" {
  ...
  password                            = jsondecode(data.aws_secretsmanager_secret_version.soasandbox-password.secret_string)["admin_server_password"]
  port                                = "1521"
  ...
  db_subnet_group_name    = aws_db_subnet_group.soa_sandbox.id
