...
resource "aws_ecs_capacity_provider" "managed" {
  name = "${local.application_data.accounts[local.environment][local.component_name].app_name}-capacity-provider-managed"
...
resource "aws_ecs_capacity_provider" "admin" {
  name = "${local.application_data.accounts[local.environment][local.component_name].app_name}-capacity-provider-admin"
  auto_scaling_group_provider {
...
resource "aws_ecs_cluster" "main" {
  name = "${local.application_data.accounts[local.environment][local.component_name].app_name}-cluster"
  setting {
...
resource "aws_ecs_task_definition" "admin" {
  family             = "${local.application_data.accounts[local.environment][local.component_name].app_name}-admin-task"
