resource "aws_iam_role" "lambda_cloudwatch_sns_role" {
  name = "${local.application_name}-${local.environment}-${local.component_name}-lambda_cloudwatch_sns_role"
...
resource "aws_lambda_function" "cloudwatch_sns" {
  ...
  source_code_hash = base64sha256(join("", local.lambda_source_cloudwatch_sns_hashes))
