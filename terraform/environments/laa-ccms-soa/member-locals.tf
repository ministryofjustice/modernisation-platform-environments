locals {
  ...
  lb_log_prefix_soa_admin_internal   = "${local.application_data.accounts[local.environment].app_name}-admin-internal-lb"
  lb_log_prefix_soa_managed_internal = "${local.application_data.accounts[local.environment].app_name}-managed-internal-lb"
}
