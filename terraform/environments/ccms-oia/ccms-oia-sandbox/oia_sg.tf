# OPA EC2 Instances Security Group

resource "aws_security_group" "first_cluster_ec2" {
...
resource "aws_security_group" "second_cluster_ec2" {
  name        = "${local.second_cluster_name}-cluster-ec2-security-group"
  description = "Controls access to the cluster EC2 instances"
  vpc_id      = data.aws_vpc.shared.id

  tags = merge(local.tags,
    { Name = lower(format("%s-ec2-sg", local.second_cluster_name)) }
  )
}
