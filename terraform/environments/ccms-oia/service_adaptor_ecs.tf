...
resource "aws_ecs_task_definition" "ecs_adaptor_task_definition" {
  ...
  container_definitions = templatefile(
    "${path.module}/templates/task_definition_service_adaptor.json.tpl",
    {
      ...
      server_opa10assess_security_user_name     = local.application_data.accounts[local.environment].server_opa10assess_security_user_name
      server_opa10assess_security_user_password = aws_secretsmanager_secret.server_opa10assess_security_user_password.arn 
      }
  )
...
resource "aws_ecs_service" "ecs_adaptor_service" {
  name            = "${local.adaptor_app_name}-service"
