...
resource "aws_athena_named_query" "main_table_connector_internal" {
  name      = lower(format("%s-%s-create-table-internal", local.connector_app_name, local.environment))
...
resource "aws_athena_named_query" "http_requests_connector_internal" {
  name      = lower(format("%s-%s-http-get-requests-internal", local.connector_app_name, local.environment))
  workgroup = aws_athena_workgroup.lb-access-logs.id
  database  = aws_athena_database.lb-access-logs.name
  query = templatefile(
...
resource "aws_athena_named_query" "main_table_opa" {
  name      = lower(format("%s-%s-create-table", local.opa_app_name, local.environment))
  workgroup = aws_athena_workgroup.lb-access-logs.id
  database  = aws_athena_database.lb-access-logs.name
  query = templatefile(
...
resource "aws_athena_named_query" "http_requests_opa" {
  name      = lower(format("%s-%s-http-get-requests", local.opa_app_name, local.environment))
  workgroup = aws_athena_workgroup.lb-access-logs.id
  database  = aws_athena_database.lb-access-logs.name
  query = templatefile(
...
resource "aws_athena_named_query" "http_requests_adaptor_internal" {
  name      = lower(format("%s-%s-http-get-requests-internal", local.adaptor_app_name, local.environment))
  workgroup = aws_athena_workgroup.lb-access-logs.id
  database  = aws_athena_database.lb-access-logs.name
  query = templatefile(
