...
locals {
  rds_sg_group_ids = compact([
    ...
    contains(["development", "production"], var.environment) ? aws_security_group.vpc_sec_group[0].id : "",
    contains(["development", "production"], var.environment) ? aws_security_group.mlra_ecs_sec_group[0].id : ""
  ])
}
