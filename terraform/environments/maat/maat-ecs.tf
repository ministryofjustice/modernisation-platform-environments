...
resource "aws_iam_role" "maat_ecs_tasks_role" {
  name = "${local.application_name}-ecs-tasks-role"
  tags = merge(
    local.tags,
    {
      Name = "${local.application_name}-ecs-tasks-role"
...
resource "aws_ecs_task_definition" "maat_ecs_task_definition" {
  ...
  execution_role_arn       = aws_iam_role.maat_ecs_tasks_role.arn
  task_role_arn            = aws_iam_role.maat_ecs_tasks_role.arn
