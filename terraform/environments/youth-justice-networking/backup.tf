...
resource "aws_backup_vault" "juniper_backup_vault" {
...
resource "aws_backup_plan" "yjaf_backup_plan" {
  ...
  rule {
    ...
    target_vault_name = aws_backup_vault.juniper_backup_vault.name
    schedule          = "cron(0 21 ? * * *)"
    start_window      = 60
    ...
    Statement = [
      {
        ...
        Resource = aws_backup_vault.juniper_backup_vault.arn
        Condition = {
          StringEquals = {
