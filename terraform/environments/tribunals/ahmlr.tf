...
#   hmlands_user_data = base64encode(templatefile("user_data.sh", {
#     cluster_name = "${local.hmlands}_app_cluster"
#   }))
#   hmlands_task_definition = templatefile("task_definition.json", {
#     app_name            = "${local.hmlands}"
#     #ecr_url             = "mcr.microsoft.com/dotnet/framework/aspnet:4.8"
#     #docker_image_tag    = "latest" 
#     #sentry_env          = local.environment
#     awslogs-group       = "${local.hmlands}-ecs-log-group"
...
# ##### EFS #####
# # resource "aws_efs_file_system" "hmlands_app_efs" {
# #   encrypted        = true
# #   kms_key_id       = data.aws_kms_key.ebs_shared.arn
# #   performance_mode = "generalPurpose"
# #   tags = merge(tomap({
# #     "Name"                 = "${local.hmlands}-app-efs"
...
# # resource "aws_efs_mount_target" "hmlands_app_efs" {
# #   file_system_id = aws_efs_file_system.hmlands_app_efs.id
# #   subnet_id      = data.aws_subnet.data_subnets_a.id
# #   security_groups = [
# #     aws_security_group.hmlands_app_efs_sg.id
# #   ]
# # }

# # resource "aws_security_group" "hmlands_app_efs_sg" {
# #   name_prefix = "${local.hmlands}-app-efs-sg-"
# #   description = "Allow inbound access from instances"
# #   vpc_id      = data.aws_vpc.shared.id

# #   tags = merge(tomap(
# #     { "Name" = "${local.hmlands}-app-efs-sg" }
# #   ), local.tags)
...
#   user_data                 = local.hmlands_user_data
#   key_name                  = ""
#   task_definition           = local.hmlands_task_definition
...
#   ec2_ingress_rules         = local.hmlands_ec2_ingress_rules
#   ec2_egress_rules          = local.hmlands_ec2_egress_rules
#   lb_tg_arn                 = aws_lb_target_group.hmlands_target_group.arn
...
#   lb_listener               = aws_lb_listener.hmlands_lb
