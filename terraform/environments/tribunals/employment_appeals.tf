...
resource "aws_ecs_cluster" "eat_cluster" {
  name = "${local.eat}_app_cluster"
  setting {
...
resource "aws_cloudwatch_log_group" "eatFamily_logs" {
  name = "/ecs/${local.eat}Family"
}

resource "aws_ecs_task_definition" "eat_task_definition" {
  family                   = "${local.eat}Family"
  requires_compatibilities = ["FARGATE"]
  network_mode             = "awsvpc"
  execution_role_arn       = aws_iam_role.eat_execution.arn
  task_role_arn            = aws_iam_role.eat_task.arn
  cpu                      = 1024
  memory                   = 2048
  container_definitions = jsonencode([
    {
      name      = "${local.eat}-container"
      ...
      logConfiguration = {
        logDriver = "awslogs"
        options = {
          awslogs-group         = "${aws_cloudwatch_log_group.eatFamily_logs.name}"
      ...
      environment = [
        {
          name  = "RDS_HOSTNAME"
          value = "${local.eat_rds_url}"
        },
        {
          name  = "RDS_PORT"
          value = "${local.eat_rds_port}"
        },
        {
          name  = "RDS_USERNAME"
          value = "${local.eat_rds_user}"
        },
        {
          name  = "RDS_PASSWORD"
          value = "${local.eat_rds_password}"
        },
        {
          name  = "DB_NAME"
          value = "${local.eat_db_name}"
        },
        {
          name  = "supportEmail"
...
resource "aws_ecs_service" "eat_ecs_service" {
  depends_on = [
    aws_lb_listener.eat_lb
  ]

  name                              = local.eat
  cluster                           = aws_ecs_cluster.eat_cluster.id
  task_definition                   = aws_ecs_task_definition.eat_task_definition.arn
  launch_type                       = "FARGATE"
  ...
  load_balancer {
    ...
    container_name   = "${local.eat}-container"
    container_port   = 80
  }
...
resource "aws_iam_role" "eat_execution" {
  name = "execution-${local.eat}"
...
resource "aws_iam_role_policy" "eat_execution" {
  name = "execution-${local.eat}"
  role = aws_iam_role.eat_execution.id
...
resource "aws_iam_role" "eat_task" {
  name = "task-${local.eat}"
...
resource "aws_iam_role_policy" "eat_task" {
  name = "task-${local.eat}"
  role = aws_iam_role.eat_task.id
...
resource "aws_security_group" "eat_ecs_service" {
