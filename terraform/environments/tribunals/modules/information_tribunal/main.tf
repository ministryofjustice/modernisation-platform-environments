...
module "dms" {
  ...
  replication_task_id         = "it-migration-task"
  #target_db_instance          = 0
  target_endpoint_id          = "it-target"
  target_database_name        = "it"
  target_server_name          = var.rds_url
  target_username             = var.rds_user
  target_password             = var.rds_password
  source_endpoint_id          = "it-source"
  source_database_name        = "it"
...
resource "null_resource" "setup_db" {
  ...
  provisioner "local-exec" {
    ...
    environment = {
      ...
      APP_FOLDER = "information_tribunal"
    }
  }
  triggers = {
