...
module "dms" {
  ...
  replication_task_id         = "ftt-migration-task"
  #target_db_instance          = 0
  target_endpoint_id          = "ftt-target"
  target_database_name        = "ftt"
  target_server_name          = var.rds_url
  target_username             = var.rds_user
  target_password             = var.rds_password
  source_endpoint_id          = "ftt-source"
  source_database_name        = "ftt"
