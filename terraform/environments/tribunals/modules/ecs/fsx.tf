...
resource "aws_directory_service_directory" "transport_ad" {
  name     = "${var.app_name}.AD"
  password = aws_secretsmanager_secret_version.sversion.secret_string
  edition  = "Standard"
  type     = "MicrosoftAD"

  vpc_settings {
    vpc_id     = var.fsx_vpc_id #data.aws_vpc.shared.id
    subnet_ids = var.fsx_subnet_ids #[data.aws_subnet.public_subnets_a.id, data.aws_subnet.public_subnets_b.id]
  }
