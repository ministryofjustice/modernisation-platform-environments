...
data "aws_vpc" "apc_vpc" {
  tags = {
    "Name" = "${var.networking[0].application}-${local.networking_environment}"
  }
}
...
data "aws_subnets" "apc_public_subnets" {
  ...
  tags = {
    Name = "${var.networking[0].application}-${local.networking_environment}-public*"
  }
}
...
data "aws_subnet" "apc_private_subnet_a" {
  ...
  tags = {
    Name = "${var.networking[0].application}-${local.networking_environment}-private-${data.aws_region.current.region}a"
  }
}
...
data "aws_subnet" "apc_private_subnet_b" {
  ...
  tags = {
    Name = "${var.networking[0].application}-${local.networking_environment}-private-${data.aws_region.current.region}b"
  }
}
