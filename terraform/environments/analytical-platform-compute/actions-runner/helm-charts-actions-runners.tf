...
resource "helm_release" "actions_runner_mojas_create_a_derived_table_emds_dev" {
  count = terraform.workspace == "analytical-platform-compute-production" ? 1 : 0

  /* https://github.com/ministryofjustice/analytical-platform-actions-runner */
  name       = "actions-runner-mojas-create-a-derived-table-emds-dev"
  repository = "oci://ghcr.io/ministryofjustice/analytical-platform-charts"
  version    = "2.330.0-5"
  chart      = "actions-runner"
  namespace  = kubernetes_namespace.actions_runners[0].metadata[0].name
...
resource "helm_release" "actions_runner_mojas_create_a_derived_table_property_prod" {
  count = terraform.workspace == "analytical-platform-compute-production" ? 1 : 0

  /* https://github.com/ministryofjustice/analytical-platform-actions-runner */
  name       = "actions-runner-mojas-cadt-property-prod"
  repository = "oci://ghcr.io/ministryofjustice/analytical-platform-charts"
  version    = "2.330.0-5"
  chart      = "actions-runner"
  namespace  = kubernetes_namespace.actions_runners[0].metadata[0].name
  values = [
    templatefile(
      "${path.module}/src/helm/values/actions-runners/create-a-derived-table/values.yml.tftpl",
      {
        ...
        github_organisation        = "moj-analytical-services"
        github_repository          = "create-a-derived-table"
        github_runner_labels       = "property-prod"
        eks_role_arn               = "arn:aws:iam::${local.environment_management.account_ids["property-cafm-data-migration-production"]}:role/create-a-derived-table"
      }
    )
  ]
}

resource "helm_release" "actions_runner_mojas_create_a_derived_table_probation_dev" {
  count = terraform.workspace == "analytical-platform-compute-production" ? 1 : 0
  ...
  values = [
    templatefile(
      "${path.module}/src/helm/values/actions-runners/create-a-derived-table/values.yml.tftpl",
      {
        ...
        github_organisation        = "moj-analytical-services"
        github_repository          = "create-a-derived-table"
        github_runner_labels       = "probation-dev"
        eks_role_arn               = "arn:aws:iam::${local.environment_management.account_ids["analytical-platform-data-engineering-production"]}:role/probation-cadet-dev"
      }
    )
  ]
}

resource "helm_release" "actions_runner_mojas_create_a_derived_table_probation_preprod" {
  count = terraform.workspace == "analytical-platform-compute-production" ? 1 : 0
  ...
  values = [
    templatefile(
      "${path.module}/src/helm/values/actions-runners/create-a-derived-table/values.yml.tftpl",
      {
        ...
        github_organisation        = "moj-analytical-services"
        github_repository          = "create-a-derived-table"
        github_runner_labels       = "probation-preprod"
        eks_role_arn               = "arn:aws:iam::${local.environment_management.account_ids["analytical-platform-data-engineering-production"]}:role/probation-cadet-preprod"
      }
    )
  ]
}

resource "helm_release" "actions_runner_mojas_create_a_derived_table_probation_prod" {
  count = terraform.workspace == "analytical-platform-compute-production" ? 1 : 0
  ...
  values = [
    templatefile(
      "${path.module}/src/helm/values/actions-runners/create-a-derived-table/values.yml.tftpl",
      {
        ...
        github_organisation        = "moj-analytical-services"
        github_repository          = "create-a-derived-table"
        github_runner_labels       = "probation-prod"
        eks_role_arn               = "arn:aws:iam::${local.environment_management.account_ids["analytical-platform-data-engineering-production"]}:role/probation-cadet-prod"
      }
    )
  ]
}
