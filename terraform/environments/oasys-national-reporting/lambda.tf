...
data "aws_iam_policy_document" "automation_lambda_assume_role" {
  statement {
    effect = "Allow"
    principals {
      type        = "Service"
...
resource "aws_iam_role" "automation_lambda" {
  name               = "${local.application_name}-automation-lambda-role"
  assume_role_policy = data.aws_iam_policy_document.automation_lambda_assume_role.json
  tags = merge(
    local.tags,
    { Name = "${local.application_name}-automation-lambda-role" }
  )
...
resource "aws_iam_policy" "automation_lambda" { #tfsec:ignore:aws-iam-no-policy-wildcards
  name = "${local.application_name}-${local.environment}-automation-lambda-policy"
  tags = merge(
    local.tags,
    { Name = "${local.application_name}-${local.environment}-automation-lambda-policy" }
  )
  policy = <<EOF
{
    ...
    "Statement": [
        ...
        },
        {
            ...
            Resource = "arn:aws:secretsmanager:*:*:secret:/github_pat/*",
        }
    ]
}
EOF
