...
module "ecs-new" {
  source        = "github.com/ministryofjustice/terraform-ecs//ecs-ec2?ref=d0d070a"
  ...
  user_data = templatefile("templates/user-data.txt", {
    CLUSTER_NAME = format("%s-new", local.application_name)
  })
  vpc_id           = data.aws_vpc.shared.id
  min_size         = local.app_data.accounts[local.environment].ec2_min_size
  max_size         = local.app_data.accounts[local.environment].ec2_max_size
  desired_capacity = local.app_data.accounts[local.environment].ec2_desired_capacity
  instance_type    = local.app_data.accounts[local.environment].instance_type
  key_name         = local.app_data.accounts[local.environment].key_name
  tags_common      = local.tags
}