...
locals {
  skip_final_snapshot = try(local.application_data.accounts[local.environment].db_skip_final_snapshot, false)
}
...
resource "aws_db_instance" "jitbit" {
  ...
  apply_immediately = true # temp switch to true for rds resize apply
