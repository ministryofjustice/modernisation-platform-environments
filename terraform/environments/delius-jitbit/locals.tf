locals {
  ...
  domain_record_sub_blue          = [for k, v in local.domain_types : v.record if k == "blue-${local.app_url}"]
  domain_record_sub_green         = [for k, v in local.domain_types : v.record if k == "green-${local.app_url}"]
