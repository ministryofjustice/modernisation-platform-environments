##################################################################################################################
# Random Secret for the DB Password to be used for installation of EDW only
...
resource "aws_db_instance" "edw_rds_instance" {
  ...
  max_allocated_storage = local.application_data.accounts[local.environment].max_allocated_storage
  ...
  storage_throughput    = local.application_data.accounts[local.environment].storage_throughput
  ...
  performance_insights_kms_key_id       = data.aws_kms_key.rds_shared.arn
