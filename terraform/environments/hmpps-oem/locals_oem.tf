locals {
  ...
  oem_share_secret_principal_ids = flatten([
    ...
    concat(
      ...
      ((contains(["development", "test"], local.environment) && contains(["delius-core-${local.environment}"], key)) || contains(["delius-mis-${local.environment}"], key)) ? ["arn:aws:iam::${value}:role/modernisation-platform-oidc-cicd"] : []
    )
  ])
