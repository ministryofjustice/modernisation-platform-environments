...
locals {
  ...
  db_secret_arn = local.environment == "development" ? try(aws_secretsmanager_secret.app_apex_dbpassword_tad.arn, "") : "arn:aws:ssm:${local.application_data.accounts[local.environment].region}:${local.env_account_id}:parameter/${local.app_db_password_name}"
