...
# Lambda Layer loaded from the apex shared S3 bucket
...
resource "aws_cloudwatch_log_subscription_filter" "database_oracle_alerts" {
  ...
  filter_pattern  = "\"ORA-\""
  destination_arn = aws_lambda_function.guardduty_slack_notify.arn

  depends_on = [aws_lambda_permission.allow_logs_invoke_cloudwatch]
}
...
resource "aws_guardduty_malware_protection_plan" "backup_lambda_protection_plan" {
  role = data.aws_iam_role.guardduty_s3_scan.arn

  protected_resource {
    s3_bucket {
      bucket_name = aws_s3_bucket.backup_lambda.id
    }
  }
  ...
  tags = merge(local.tags,
    { Name = lower(format("s3-%s-%s-backup-lambda-guardduty-protection-plan", local.application_name, local.environment)) }
  )
