resource "helm_release" "aws_load_balancer_controller" {
  ...
  set = [
    ...
    {
      # TODO: if this works figure out how to get the vpc id instead of the name
      name  = "vpcId"
      value = "vpc-0fe6f927602ffdbc0"
    },
    {
...
resource "kubernetes_manifest" "test_http_route" {
  manifest = yamldecode(file("./manifests/http-route.yaml"))
}
    ...
    "Statement" : [
      {
        "Effect" : "Allow",
        "Principal" : {
          "Federated" : "arn:aws:iam::557395370360:oidc-provider/oidc.eks.eu-west-2.amazonaws.com/id/C8A7610FFC08D9F3B7D5A4C57EF6049D"
        },
        "Action" : "sts:AssumeRoleWithWebIdentity",
        "Condition" : {
          "StringEquals" : {
            "oidc.eks.eu-west-2.amazonaws.com/id/C8A7610FFC08D9F3B7D5A4C57EF6049D:aud" : "sts.amazonaws.com",
            "oidc.eks.eu-west-2.amazonaws.com/id/C8A7610FFC08D9F3B7D5A4C57EF6049D:sub" : "system:serviceaccount:kube-system:aws-load-balancer-controller"
          }
        }
      }
    ]
...
resource "kubernetes_manifest" "lbc_service_account" {
  manifest = yamldecode(file("./manifests/lbc-service-account.yaml"))
}
...
resource "aws_iam_role_policy_attachment" "lbc-role-policy-attach" {
  role       = aws_iam_role.lbc_role.name
  policy_arn = aws_iam_policy.lbc_policy.arn
}

output "vpc_id_debug" {
  value = local.cp_vpc_name
}
