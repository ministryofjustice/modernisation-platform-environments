###############################################################################
# Unified Cluster Components Module
#
# Platform components installed on every cluster (hub and spoke):
# - OPA Gatekeeper (policy engine)
# - AWS Load Balancer Controller (self-managed, for Gateway API)
# - WAF Web ACL (OWASP baseline with encrypted logging)
###############################################################################
...
###############################################################################
# WAF Web ACL — OWASP Top 10 baseline + Encrypted Logging
###############################################################################
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Condition = {
          ArnLike = {
            "kms:EncryptionContext:aws:logs:arn" = "arn:aws:logs:eu-west-2:${var.account_id}:log-group:aws-waf-logs-${var.cluster_name}"
          }
        }
      }
    ]
