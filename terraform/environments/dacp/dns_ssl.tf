...
resource "aws_acm_certificate" "external" {
  domain_name       = local.is-production ? "divorce-section-search.service.justice.gov.uk" : "modernisation-platform.service.justice.gov.uk"
...
resource "aws_route53_record" "external-prod" {
  ...
  alias {
    name                   = aws_lb.dacp_lb.dns_name
    zone_id                = aws_lb.dacp_lb.zone_id
