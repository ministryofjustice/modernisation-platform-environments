...
resource "aws_acm_certificate" "external" {
  domain_name       = local.is-production ? "parental-responsibility-agreement.service.justice.gov.uk" : "modernisation-platform.service.justice.gov.uk"
...
resource "aws_route53_record" "external-prod" {
  ...
  alias {
    name                   = aws_lb.pra_lb.dns_name
    zone_id                = aws_lb.pra_lb.zone_id
