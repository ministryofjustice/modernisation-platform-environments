...
resource "aws_security_group" "modernisation_pra_access" {
  ...
  name        = "modernisation_pra_access-${local.environment}"
  description = "Allow pra on modernisation platform to access the source database"

  ingress {
    from_port   = 5432
    to_port     = 5432
    protocol    = "tcp"
    description = "Allow pra on modernisation platform to connect to source database"
...
resource "null_resource" "setup_source_rds_security_group" {
  provisioner "local-exec" {
    ...
    environment = {
      RDS_SECURITY_GROUP            = aws_security_group.modernisation_pra_access.id
...
resource "null_resource" "setup_dev_db" {
  ...
  provisioner "local-exec" {
    ...
    environment = {
      ...
      PRA_DB_USERNAME = aws_db_instance.ncas_db.username
      PRA_DB_PASSWORD = random_password.password.result
    }
  }
  triggers = {
