...
data "aws_iam_policy_document" "airflow_cadt_cross_assume" {
  #checkov:skip=CKV_AWS_110:Ensure IAM policies does not allow privilege escalation
  #checkov:skip=CKV_AWS_358:OIDC trust policies only allows actions from a specific known organization Already
  #checkov:skip=CKV_AWS_107:Ensure IAM policies does not allow credentials exposure
  #checkov:skip=CKV_AWS_111:Ensure IAM policies does not allow write access without constraints
...
resource "aws_iam_role" "airflow_cadt_cross_assume_role" {
  ...
  name                  = "airflow-dev-cadet-deploy-emds${local.suffix}"
  description           = "Role to use for Airflow CADT"
  assume_role_policy    = data.aws_iam_policy_document.airflow_cadt_cross_assume.json
