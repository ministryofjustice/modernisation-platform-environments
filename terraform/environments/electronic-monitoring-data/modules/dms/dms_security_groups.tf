resource "aws_security_group" "dms_ri_security_group" {
    ...
    description = "Secuity Group having relevant DMS access"
    vpc_id      = var.dms_vpc_id # data.aws_vpc.shared.id
...
resource "aws_vpc_security_group_ingress_rule" "dms_to_rds_sg_rule" {
    security_group_id = var.rds_db_security_group_id # aws_security_group.db.id
