locals {
  table_mappings_dir = "${path.root}/terraform/environments/electronic-monitoring-data"
}

# Create a new replication task
resource "aws_dms_replication_task" "dms_db_migration_task" {
  count = fileexists(
      "${local.table_mappings_dir}/dms_${var.database_name}_task_tables_selection.json"
    ) && !fileexists(
      "${local.table_mappings_dir}/dms_${var.database_name}_task_transformations.json"
    ) ? 1 : 0
