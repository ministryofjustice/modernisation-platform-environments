...
while true; do
  if [[ -n "$next_token" ]]; then
    response="$(aws glue get-tables --region "$region" --database-name "$database_name" --query "{TableList: TableList[?Parameters.table_type=='ICEBERG'], NextToken: NextToken}" --next-token "$next_token" --output json)"
  else
    response="$(aws glue get-tables --region "$region" --database-name "$database_name" --query "{TableList: TableList[?Parameters.table_type=='ICEBERG'], NextToken: NextToken}" --output json)"
  fi
