...
while true; do
  if [[ -n "$next_token" ]]; then
    response="$(aws glue get-tables --region "$region" --database-name "$database_name" --query "{TableList: TableList[?Parameters.presto_view!='true' && Parameters.classification!='parquet'], NextToken: NextToken}" --next-token "$next_token" --output json)"
  else
    response="$(aws glue get-tables --region "$region" --database-name "$database_name" --query "{TableList: TableList[?Parameters.presto_view!='true' && Parameters.classification!='parquet'], NextToken: NextToken}" --output json)"
  fi
