...
DEFAULT_INPUTS_LIST = ["JOB_NAME",
                       ...
                       "repartition_factor",
                       "max_table_size_mb",
                       "trim_rds_df_str_col_list",
                       "rds_tbl_primary_key_list"
                       ]
...
NVL_DTYPE_DICT = {'string': "''", 'tinyint': 0, 'smallint': 0, 'int': 0, 'bigint':0,
                  'double': 0, 'float': 0, 
                  'boolean': False,
...
def get_altered_df_schema_object(in_df_rds: DataFrame, in_transformed_column_list):
    ...
    for colmn in in_transformed_column_list:
        if colmn not in rds_df_column_list:
            LOGGER.error(f"""Given transformed column '{colmn}' is not an existing RDS-DB-Table-Column.""")
            sys.exit(1)
...
def process_dv_for_table(rds_db_name, rds_tbl_name, total_files, total_size_mb, input_repartition_factor) -> DataFrame:
    ...
    if tbl_prq_s3_folder_path is not None:
        ...
        rds_tbl_primary_key_list = [f"""{column.strip().strip("'").strip('"')}""" 
                                    for column in args['rds_tbl_primary_key_list'].split(",")]
        
        for rds_column in df_rds_temp.columns:
            if rds_column in rds_tbl_primary_key_list:
                continue

            temp_select_list = rds_tbl_primary_key_list.append(rds_column)
            df_rds_temp = df_rds.select(*temp_select_list)

            # -------------------------------------------------------
            trim_rds_df_str_col_list = [f"""{column.strip().strip("'").strip('"')}""" 
                                        for column in args.get('trim_rds_df_str_col_list', '').split(",")]
            
            trim_col_msg = f"""Given -> trim_rds_df_str_columns = {args["trim_rds_df_str_columns"]}, 
                               {type(args["trim_rds_df_str_columns"])}""".strip()
            if rds_column in trim_rds_df_str_col_list:
                LOGGER.info(trim_col_msg)
                    
                df_rds_temp_t1 = df_rds_temp.transform(trim_rds_df_str_columns)
                additional_validation_msg = " - [After trimming RDS-DB-string column(s) spaces]"
            ...
            if df_rds_prq_subtract_row_count == 0:
                df_temp = df_dv_output.selectExpr("current_timestamp as run_datetime",
                                                  "'' as json_row",
                                                  f"""'{rds_tbl_name}.{rds_column} - Validated.{additional_validation_msg}' as validation_msg""",
                                                  f"""'{rds_db_name}' as database_name""",
                                                  f"""'{db_sch_tbl}' as full_table_name""",
                                                  """'False' as table_in_ap"""
                                                  )
            ...
            df_rds_prq_subtract_transform.unpersist()
...
if __name__ == "__main__":
    ...
    # -------------------------------------------------------
    if args.get("select_rds_db_table", None) is None:
        LOGGER.error(f"""'select_rds_db_table' runtime input is missing ! """)
        sys.exit(1)

    given_rds_sqlserver_table = args["select_rds_db_table"]
    if not rds_sqlserver_db_tbl_list:
        ...
        LOGGER.error(f"""'{given_rds_sqlserver_table}' - is not an existing table.""")
        sys.exit(1)
    # -------------------------------------------------------
