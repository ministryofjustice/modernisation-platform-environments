...
resource "aws_athena_workgroup" "data_validation_db" {
  name        = format("%s-data-validation", local.env_account_id)
  description = "An Athena workgroup for the data validation db, dumps to: ${module.s3-athena-bucket.bucket.id}"
  state       = "ENABLED"
  ...
  configuration {
    ...
    result_configuration {
      output_location = "s3://${module.s3-athena-bucket.bucket.id}/output/data_validation/"
