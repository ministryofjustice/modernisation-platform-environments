...
# ------------------------------------------------------------------------
# Dagster S3 Bucket resources
# ------------------------------------------------------------------------

module "dagster-bucket" {
  source = "github.com/ministryofjustice/modernisation-platform-terraform-s3-bucket?ref=cadab51"

  bucket_prefix      = "dagster-output-store-"
  versioning_enabled = true
...
module "dagster-bucket-logs" {
  source = "./modules/s3_log_bucket"

  source_bucket = module.dagster-bucket.bucket
  account_id    = data.aws_caller_identity.current.account_id
  local_tags    = local.tags
}