...
locals {
  serco_fms_key_distribution_lambda_names = {
    ...
    access_observer = module.serco_fms_key_access_observer.lambda_function_name
    dashboard       = module.serco_fms_key_distribution_dashboard.lambda_function_name
  }
}
...
resource "aws_cloudwatch_metric_alarm" "serco_fms_key_distribution_errors" {
  ...
  alarm_name = "serco_fms_key_distribution_${each.key}_errors_${local.environment_shorthand}"
  alarm_description = "Triggered when the Serco FMS ${each.key} Lambda records an error"
