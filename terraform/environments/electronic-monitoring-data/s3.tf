locals {
  ...
  bucket_logging_source_arns = concat(
    [for bucket in local.buckets_to_log : bucket.arn],
...
# Backup  of the staging prefix in cadt bucket
...
module "s3-serco-fms-key-distribution-bucket" {
  ...
  ownership_controls = "BucketOwnerEnforced"
  custom_kms_key     = aws_kms_key.serco_fms_key_distribution.arn
  ...
  lifecycle_rule = [
    ...
    },
    {
      ...
      enabled = "Enabled"
      prefix  = "${local.serco_fms_key_distribution_passwords_prefix}/"
...
resource "aws_s3_bucket_logging" "serco_fms_key_distribution" {
  ...
  target_bucket = module.s3-logging-bucket.bucket.id
  target_prefix = "logs/serco-fms-key-distribution/"
