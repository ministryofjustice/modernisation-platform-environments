...
resource "aws_api_gateway_method" "update_p1_export_post" {
  count = local.is-development ? 1 : 0
  rest_api_id          = aws_api_gateway_rest_api.update_p1_export[0].id
  resource_id          = aws_api_gateway_resource.update_p1_export[0].id
  http_method          = "POST"
...
resource "aws_api_gateway_integration" "update_p1_export_lambda_post" {
  ...
  rest_api_id = aws_api_gateway_rest_api.update_p1_export[0].id
  resource_id = aws_api_gateway_resource.update_p1_export[0].id
  http_method = aws_api_gateway_method.update_p1_export_post[0].http_method
...
resource "aws_api_gateway_deployment" "update_p1_export" {
  count = local.is-development ? 1 : 0
  depends_on = [aws_api_gateway_integration.update_p1_export_lambda_post[0],]
