...
def invoke_lambda_for_file(file_key, lambda_function_name):
    """
    Invoke the Lambda function for a given file.
    """
    lambda_client = boto3.client("lambda", region_name="eu-west-2")
    response = lambda_client.invoke(
        FunctionName=lambda_function_name,
        InvocationType="Event",  # Asynchronous invocation
        Payload='{"file_key": "' + file_key + '"}',  # Sending file key as payload
    )
    print(f"Invoked Lambda '{lambda_function_name}' for file: {file_key}")
