...
resource "aws_iam_role" "create_athena_external_tables_glue" {
    name = "create_athena_external_tables_glue"
    assume_role_policy = data.aws_iam_policy_document.glue_assume_role.json
}


resource "aws_iam_role_policy_attachment" "get_glue_connections_and_tables" {
    role = aws_iam_role.create_athena_external_tables_glue.name
