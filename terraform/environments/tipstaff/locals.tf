...
locals {
  ...
  domain_name_main   = local.is-production ? [for k, v in local.domain_types : v.name if k == "tipstaff.service.justice.gov.uk"] : [for k, v in local.domain_types : v.name if k == "modernisation-platform.service.justice.gov.uk"]
  domain_name_sub    = local.is-production ? [for k, v in local.domain_types : v.name if k != "tipstaff.service.justice.gov.uk"] : [for k, v in local.domain_types : v.name if k != "modernisation-platform.service.justice.gov.uk"]
  domain_record_main = local.is-production ? [for k, v in local.domain_types : v.record if k == "tipstaff.service.justice.gov.uk"] : [for k, v in local.domain_types : v.record if k == "modernisation-platform.service.justice.gov.uk"]
  domain_record_sub  = local.is-production ? [for k, v in local.domain_types : v.record if k != "tipstaff.service.justice.gov.uk"] : [for k, v in local.domain_types : v.record if k != "modernisation-platform.service.justice.gov.uk"]
