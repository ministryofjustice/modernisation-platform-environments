resource "aws_dms_endpoint" "source" {
  ...
  username                    = jsondecode(data.aws_secretsmanager_secret_version.dms_source_credentials[0].secret_string)["DTS-STAGING-DB-MASTER-USER"]
  password                    = jsondecode(data.aws_secretsmanager_secret_version.dms_source_credentials[0].secret_string)["DTS-STAGING-DB-MASTER-PASSWORD"]
  port                        = 5432
  server_name                 = jsondecode(data.aws_secretsmanager_secret_version.dms_source_credentials[0].secret_string)["DTS-STAGING-DB-HOSTNAME"]
  ssl_mode                    = "none"
