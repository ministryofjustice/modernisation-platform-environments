...
data "aws_secretsmanager_secret_version" "db_credentials" {
  count     = local.application_data.accounts[local.environment] == "development" ? 1 : 0
  secret_id = data.aws_secretsmanager_secret.tipstaff_dev_db_secrets[count.index].id
}
...
resource "aws_secretsmanager_secret_version" "secrets_version_username" {
  count         = local.application_data.accounts[local.environment] == "preproduction" ? 1 : 0
  secret_id     = aws_secretsmanager_secret.tipstaff_pre_prod_db_secrets[count.index].id
  secret_string = random_string.username[count.index].result
}
