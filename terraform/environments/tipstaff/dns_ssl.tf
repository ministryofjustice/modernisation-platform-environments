...
resource "aws_route53_record" "inner_validation" {
  provider = aws.core-vpc
  for_each = {
    for dvo in aws_acm_certificate.inner.domain_validation_options : dvo.domain_name => {
...
resource "aws_acm_certificate_validation" "tipstaff_lb_cert_validation" {
  ...
  validation_record_fqdns = [for record in aws_route53_record.inner_validation : record.fqdn]
}
