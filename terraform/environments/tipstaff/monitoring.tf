...
resource "aws_cloudwatch_metric_alarm" "ddos_attack_external" {
  ...
  alarm_actions       = [aws_sns_topic.tipstaff_ddos_alarm]
  dimensions = {
...
resource "aws_sns_topic" "tipstaff_ddos_alarm" {
...
module "pagerduty_core_alerts" {
  depends_on = [
    aws_sns_topic.tipstaff_ddos_alarm
  ]
  source                    = "github.com/ministryofjustice/modernisation-platform-terraform-pagerduty-integration?ref=v2.0.0"
  sns_topics                = [aws_sns_topic.tipstaff_ddos_alarm.name]
