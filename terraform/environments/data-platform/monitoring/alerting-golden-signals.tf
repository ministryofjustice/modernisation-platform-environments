locals {
  ...
  alerting_golden_signals = {
    ...
    litellm_provider_state   = { group = "AI Gateway", datasource_type = "prometheus", expr = "min by (api_provider) ((sum by (api_provider, litellm_model_name) (rate(litellm_deployment_failure_responses_total{api_key_alias=\"litellm-internal-health-check\",container=\"litellm\"}[5m])) or (sum by (api_provider, litellm_model_name) (rate(litellm_deployment_total_requests_total{api_key_alias=\"litellm-internal-health-check\",container=\"litellm\"}[5m])) * 0)) / sum by (api_provider, litellm_model_name) (rate(litellm_deployment_total_requests_total{api_key_alias=\"litellm-internal-health-check\",container=\"litellm\"}[5m])) * 100)", metric = "litellm_provider_health_check_failure_rate_pct", type = "gt", dim_key = "", ok_when_nodata = true, warning = "litellm_provider_state_warn", critical = "litellm_provider_state_crit" }
    ...
    litellm_provider_state_admin   = { group = "AI Gateway", datasource_type = "prometheus", expr = "min by (api_provider) ((sum by (api_provider, litellm_model_name) (rate(litellm_deployment_failure_responses_total{api_key_alias=\"litellm-internal-health-check\",container=\"litellm-admin\"}[5m])) or (sum by (api_provider, litellm_model_name) (rate(litellm_deployment_total_requests_total{api_key_alias=\"litellm-internal-health-check\",container=\"litellm-admin\"}[5m])) * 0)) / sum by (api_provider, litellm_model_name) (rate(litellm_deployment_total_requests_total{api_key_alias=\"litellm-internal-health-check\",container=\"litellm-admin\"}[5m])) * 100)", metric = "litellm_provider_health_check_failure_rate_pct", type = "gt", dim_key = "", ok_when_nodata = true, warning = "litellm_provider_state_admin_warn", critical = "litellm_provider_state_admin_crit" }
