locals {
  ...
  alerting_golden_signals = {
    ...
    litellm_model_no_healthy_deployments = { group = "AI Gateway", datasource_type = "prometheus", expr = "count by (litellm_model_name) (litellm_deployment_cooled_down_total{container=\"litellm\"} > 0) - (count by (litellm_model_name) (litellm_deployment_total_requests_total{container=\"litellm\"} offset 5m > 0) or (count by (litellm_model_name) (litellm_deployment_cooled_down_total{container=\"litellm\"} > 0) * 0))", metric = "litellm_model_deployments_unhealthy_delta", type = "gt", dim_key = "", ok_when_nodata = true, critical = "litellm_model_no_healthy_deployments_crit" }
