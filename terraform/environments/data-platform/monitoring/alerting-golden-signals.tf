locals {
  ...
  alerting_golden_signals = {
    ...
    # ── AI Gateway (LiteLLM) ──────────────────────────────────────────────────
    litellm_deployment_state_warning  = { group = "AI Gateway", datasource_type = "prometheus", expr = "(sum by (litellm_model_name, requested_model) (rate(litellm_deployment_failure_responses_total{api_key_alias=\"litellm-internal-health-check\",container=\"litellm\"}[5m])) or (sum by (litellm_model_name, requested_model) (rate(litellm_deployment_total_requests_total{api_key_alias=\"litellm-internal-health-check\",container=\"litellm\"}[5m])) * 0)) / sum by (litellm_model_name, requested_model) (rate(litellm_deployment_total_requests_total{api_key_alias=\"litellm-internal-health-check\",container=\"litellm\"}[5m])) * 100", metric = "litellm_deployment_health_check_failure_rate_pct", type = "gt", dim_key = "", ok_when_nodata = true, warning = "litellm_deployment_state_warn" }
    litellm_deployment_state_critical = { group = "AI Gateway", datasource_type = "prometheus", expr = "(count((sum by (litellm_model_name, requested_model) (rate(litellm_deployment_failure_responses_total{api_key_alias=\"litellm-internal-health-check\",container=\"litellm\"}[5m]))) / (sum by (litellm_model_name, requested_model) (rate(litellm_deployment_total_requests_total{api_key_alias=\"litellm-internal-health-check\",container=\"litellm\"}[5m]))) > 0) or vector(0)) / count(sum by (litellm_model_name, requested_model) (rate(litellm_deployment_total_requests_total{api_key_alias=\"litellm-internal-health-check\",container=\"litellm\"}[5m]))) * 100", metric = "litellm_deployment_models_failing_pct", type = "gt", dim_key = "", ok_when_nodata = true, critical = "litellm_deployment_state_crit" }
    ...
    litellm_deployment_state_admin_warning  = { group = "AI Gateway", datasource_type = "prometheus", expr = "(sum by (litellm_model_name, requested_model) (rate(litellm_deployment_failure_responses_total{api_key_alias=\"litellm-internal-health-check\",container=\"litellm-admin\"}[5m])) or (sum by (litellm_model_name, requested_model) (rate(litellm_deployment_total_requests_total{api_key_alias=\"litellm-internal-health-check\",container=\"litellm-admin\"}[5m])) * 0)) / sum by (litellm_model_name, requested_model) (rate(litellm_deployment_total_requests_total{api_key_alias=\"litellm-internal-health-check\",container=\"litellm-admin\"}[5m])) * 100", metric = "litellm_deployment_health_check_failure_rate_pct", type = "gt", dim_key = "", ok_when_nodata = true, warning = "litellm_deployment_state_admin_warn" }
    litellm_deployment_state_admin_critical = { group = "AI Gateway", datasource_type = "prometheus", expr = "(count((sum by (litellm_model_name, requested_model) (rate(litellm_deployment_failure_responses_total{api_key_alias=\"litellm-internal-health-check\",container=\"litellm-admin\"}[5m]))) / (sum by (litellm_model_name, requested_model) (rate(litellm_deployment_total_requests_total{api_key_alias=\"litellm-internal-health-check\",container=\"litellm-admin\"}[5m]))) > 0) or vector(0)) / count(sum by (litellm_model_name, requested_model) (rate(litellm_deployment_total_requests_total{api_key_alias=\"litellm-internal-health-check\",container=\"litellm-admin\"}[5m]))) * 100", metric = "litellm_deployment_models_failing_pct", type = "gt", dim_key = "", ok_when_nodata = true, critical = "litellm_deployment_state_admin_crit" }
