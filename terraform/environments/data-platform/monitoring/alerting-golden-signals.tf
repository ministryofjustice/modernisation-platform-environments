locals {
  ...
  alerting_golden_signals = {
    ...
    litellm_deployment_state_critical = { group = "AI Gateway", datasource_type = "prometheus", expr = "(count((sum by (litellm_model_name, requested_model) (increase(litellm_deployment_failure_responses_total{api_key_alias=\"litellm-internal-health-check\",container=\"litellm\"}[5m]))) / clamp_min(sum by (litellm_model_name, requested_model) (increase(litellm_deployment_total_requests_total{api_key_alias=\"litellm-internal-health-check\",container=\"litellm\"}[5m])), 1) > 0) or vector(0)) / count(sum by (litellm_model_name, requested_model) (increase(litellm_deployment_total_requests_total{api_key_alias=\"litellm-internal-health-check\",container=\"litellm\"}[5m]))) * 100", metric = "litellm_deployment_models_failing_pct", type = "gt", dim_key = "", ok_when_nodata = true, critical = "litellm_deployment_state_crit" }
    litellm_provider_state            = { group = "AI Gateway", datasource_type = "prometheus", expr = "(count(litellm_provider_health_status{container=\"litellm\",api_provider=\"bedrock\"} == 0) or vector(0)) / clamp_min((count(litellm_provider_health_status{container=\"litellm\",api_provider=\"bedrock\"}) or vector(0)), 1) * 100", metric = "litellm_provider_models_failing_pct", type = "gt", dim_key = "", ok_when_nodata = true, warning = "litellm_provider_state_warn", critical = "litellm_provider_state_crit" }
  }
}