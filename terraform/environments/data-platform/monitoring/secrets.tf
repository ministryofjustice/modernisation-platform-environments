...
# admin), so a service account must be created in Grafana once via an SSO admin
# and its token stored here. Created with a placeholder value and populated
# out-of-band, then read back via the data source in data.tf.
