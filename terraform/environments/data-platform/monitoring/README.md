...
alerting-rules.tf              The engine: expands golden signals into one
                                ...
                                and builds the CloudWatch/Prometheus query pipeline.
...
This same entry is where every other account-level lives — resource lists for the `dim_key` fan-out, threshold overrides, disabled rules, and query settings. Every field below is optional; add only what you need. See [Account configuration fields](#account-configuration-fields-alerts_configured_accounts)
