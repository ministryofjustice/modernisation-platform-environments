  ...
  values = [
    templatefile(
      "${path.module}/src/helm/values/${local.component_name}-configuration/values.yml.tftpl",
      {
        ...
        guardrail_diagnostics_enabled  = local.guardrail_diagnostics_enabled
...
resource "helm_release" "litellm_guardrail_diagnostics" {
  count = local.guardrail_diagnostics_enabled ? 1 : 0

  name       = "litellm-guardrail-diagnostics"
  repository = "oci://ghcr.io/berriai"
  version    = local.environment_configuration.litellm_version
  chart      = "litellm-helm"
  namespace  = local.component_name
  values = [
    templatefile(
      "${path.module}/src/helm/values/litellm-guardrail-diagnostics/values.yml.tftpl",
      {
        # Kubernetes
        imageRepository    = "ghcr.io/berriai/litellm-non_root"
        imageTag           = local.environment_configuration.litellm_version
        serviceAccountName = kubernetes_service_account_v1.ai_gateway.metadata[0].name
        ingressHostname    = local.guardrail_diagnostics_hostname
        ...
        # Diagnostics
        guardrailDiagnosticsBucket    = module.guardrail_diagnostics[0].s3_bucket_id
        guardrailDiagnosticsRegion    = data.aws_region.current.region
        guardrailDiagnosticsKmsKeyArn = module.ai_gateway_guardrail_diagnostics_kms_key[0].key_arn
