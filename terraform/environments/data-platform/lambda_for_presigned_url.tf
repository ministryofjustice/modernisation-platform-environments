...
module "data_product_presigned_url_lambda" {
  ...
  allowed_triggers = {
    ...
    AllowExecutionFromAPIGateway = {
      ...
      source_arn    = "arn:aws:execute-api:${local.region}:${local.account_id}:${aws_api_gateway_rest_api.data_platform.id}/*/${aws_api_gateway_method.presigned_url.http_method}${aws_api_gateway_resource.presigned_url.path}"
    }
  }
