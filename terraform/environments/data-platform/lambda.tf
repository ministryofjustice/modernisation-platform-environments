...
module "data_product_delete_schema_lambda" {
  source                         = "github.com/ministryofjustice/modernisation-platform-terraform-lambda-function?ref=a4392c1" # ref for V2.1
  application_name               = "data_product_delete_schema"
  tags                           = local.tags
  description                    = "Lambda to delete all versions of the json schema files and associated data for a data product schema"
  role_name                      = "data_product_schema_lambda_role_${local.environment}"
  policy_json                    = data.aws_iam_policy_document.iam_policy_document_for_delete_schema_lambda.json
  policy_json_attached           = true
  function_name                  = "data_product_delete_schema_${local.environment}"
  create_role                    = true
  reserved_concurrent_executions = 1

  image_uri    = "374269020027.dkr.ecr.eu-west-2.amazonaws.com/data-platform-delete-schema-lambda-ecr-repo:${local.delete_schema_version}"
  timeout      = 600
  tracing_mode = "Active"
  memory_size  = 128
  ...
  allowed_triggers = {
    ...
    AllowExecutionFromAPIGateway = {
      ...
      function_name = "data_product_create_metadata_${local.environment}"
      principal     = "apigateway.amazonaws.com"
      source_arn    = "arn:aws:execute-api:${local.region}:${local.account_id}:${aws_api_gateway_rest_api.data_platform.id}/*/${aws_api_gateway_method.delete_schema_for_data_product_table_name.http_method}${aws_api_gateway_resource.schema_for_data_product_table_name.path}"
    }
  }
}
