...
module "data_product_delete_schema_lambda" {
  ...
  tags                           = local.tags
  description                    = "Lambda to delete all versions of the json schema file and associated data for a data product"
  role_name                      = "data_product_schema_lambda_role_${local.environment}"
  ...
  image_uri    = "374269020027.dkr.ecr.eu-west-2.amazonaws.com/data-platform-delete-schema-lambda-ecr-repo:${local.create_schema_version}"
  timeout      = 600
  tracing_mode = "Active"
  memory_size  = 128
