...
module "delete_data_product_lambda" {
  source                         = "github.com/ministryofjustice/modernisation-platform-terraform-lambda-function?ref=a4392c1" # ref for V2.1
  application_name               = "delete_data_product"
  tags                           = local.tags
  description                    = "Delete a data product"
  role_name                      = "delete_data_product_role_${local.environment}"
  policy_json                    = data.aws_iam_policy_document.iam_policy_document_for_delete_data_product_lambda.json
  policy_json_attached           = true
  function_name                  = "delete_data_product_${local.environment}"
  create_role                    = true
  reserved_concurrent_executions = 1

  image_uri    = "374269020027.dkr.ecr.eu-west-2.amazonaws.com/data-platform-delete-data-product-lambda-ecr-repo:${local.delete_data_product_version}"
  timeout      = 600
  tracing_mode = "Active"
  memory_size  = 128
