...
module "external_secrets_iam_role" {
  ...
  external_secrets_secrets_manager_arns = [for component in jsondecode(data.http.environments_file.response_body) : "arn:aws:secretsmanager:*:*:secret:${component.name}/*"]
