...
data "aws_secretsmanager_secret_version" "litellm_license" {
  secret_id = tostring(module.litellm_license_secret.secret_id)
}

data "aws_secretsmanager_secret_version" "litellm_entra_id" {
  secret_id = tostring(module.litellm_entra_id_secret.secret_id)
}

data "aws_secretsmanager_secret_version" "justiceai_azure_openai" {
  secret_id = tostring(module.justiceai_azure_openai_secret.secret_id)
}

data "aws_secretsmanager_secret_version" "azure_openai_secret" {
  secret_id = tostring(module.azure_openai_secret.secret_id)
}
