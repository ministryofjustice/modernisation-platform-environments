...
resource "aws_cloudwatch_metric_alarm" "ecs_cpu_alarm" {
  ...
  alarm_actions       = [aws_sns_topic.cst_utilisation_alarm[0].arn]
  dimensions = {
    ClusterName = aws_ecs_cluster.cst_cluster.name
  }
}
...
resource "aws_sns_topic" "cst_utilisation_alarm" {
  count = local.is-development ? 0 : 1
  name  = "cst_utilisation_alarm"
}
