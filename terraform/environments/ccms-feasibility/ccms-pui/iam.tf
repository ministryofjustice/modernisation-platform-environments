...
# PUI task role for S3 document bucket access

resource "aws_iam_role" "pui_task" {
  name = "${local.component_name}-${local.env_label}-task-role"
...
resource "aws_iam_policy" "pui_s3" {
  name = "${local.component_name}-${local.env_label}-s3-policy"
    ...
    Statement = [
      {
        ...
        Resource = module.s3_docs.bucket.arn
      },
      {
        Effect   = "Allow"
...
resource "aws_iam_role_policy_attachment" "pui_s3" {
  role       = aws_iam_role.pui_task.name
  policy_arn = aws_iam_policy.pui_s3.arn
}
