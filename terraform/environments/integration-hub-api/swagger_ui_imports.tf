locals {
  enable_legacy_swagger_ui_imports = false
...
import {
  for_each = local.environment == "development" && local.enable_legacy_swagger_ui_imports ? { docs = true } : {}
...
import {
  ...
  id       = "${local.resource_name_prefix}-docs"
}

import {
  for_each = local.environment == "development" && local.enable_legacy_swagger_ui_imports ? { docs = true } : {}
  to       = module.lambda_api_docs.aws_iam_role_policy.additional_inline[0]
  id       = "${local.resource_name_prefix}-docs:${local.resource_name_prefix}-docs-inline"
}

import {
  for_each = local.environment == "development" && local.enable_legacy_swagger_ui_imports ? { docs = true } : {}
  to       = module.lambda_api_docs.aws_iam_role_policy.logs[0]
  id       = "${local.resource_name_prefix}-docs:${local.resource_name_prefix}-docs-logs"
}

import {
  for_each = local.environment == "development" && local.enable_legacy_swagger_ui_imports ? { docs = true } : {}
  to       = module.lambda_api_docs.aws_cloudwatch_log_group.lambda[0]
  id       = "/aws/lambda/${local.resource_name_prefix}-docs"
}
