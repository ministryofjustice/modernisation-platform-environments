...
module "dynamodb_auth_roles" {
  ...
  billing_mode = "PAY_PER_REQUEST"
  hash_key     = "role_name"
  attributes   = [{ name = "role_name", type = "S" }]
  table_class  = "STANDARD"
  timeouts     = { create = "60m", delete = "60m", update = "60m" }
  tags         = var.tags
}
...
module "dynamodb_auth_principals" {
  ...
  attributes   = [{ name = "auth_lookup_key", type = "S" }]
  table_class  = "STANDARD"
  timeouts     = { create = "60m", delete = "60m", update = "60m" }
  tags         = var.tags
}
...
module "dynamodb_multipart_uploads" {
  ...
  attributes         = [{ name = "transfer_ticket", type = "S" }]
  table_class        = "STANDARD"
  timeouts           = { create = "60m", delete = "60m", update = "60m" }
  ttl_enabled        = true
...
resource "aws_dynamodb_table_item" "transfer_client" {
  ...
  item = jsonencode({
    ...
    key_prefix            = { S = coalesce(each.value.key_prefix, each.key) }
    max_upload_size_bytes = { N = tostring(each.value.max_upload_size_bytes) }
    allowed_content_types = { L = [for value in each.value.allowed_content_types : { S = value }] }
  })
}
...
resource "aws_dynamodb_table_item" "auth_role" {
  ...
  hash_key   = "role_name"
  item = jsonencode({
    role_name          = { S = each.key }
    allowed_client_ids = { L = [for client_id in each.value.allowed_client_ids : { S = client_id }] }
  })
}
