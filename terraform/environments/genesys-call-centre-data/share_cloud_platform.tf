locals {
  ...
  glue_arns = local.is-production ? [
    ...
    "arn:aws:glue:${data.aws_region.current.name}:${local.env_account_id}:database/call_centre_laa_${local.env_}dbt",
    "arn:aws:glue:${data.aws_region.current.name}:${local.env_account_id}:table/call_centre_laa_${local.env_}dbt/*"
    ] : [
    "arn:aws:glue:${data.aws_region.current.name}:${local.env_account_id}:catalog",
    "arn:aws:glue:${data.aws_region.current.name}:${local.env_account_id}:database/call_centre_laa_${local.env_}dbt",
    "arn:aws:glue:${data.aws_region.current.name}:${local.env_account_id}:table/call_centre_laa_${local.env_}dbt/*"
...
variable "cloud-platform-iam-dev" {
  ...
  default     = "arn:aws:iam::211125476974:role/cloud-platform-irsa-6ab6c596b45e90b3-live"
}

variable "cloud-platform-iam-preprod" {
  type        = string
  description = "IAM role that our API in Cloud Platform will use to connect to this role."
  default     = "arn:aws:iam::211125476974:role/cloud-platform-irsa-bca231f5681d29c6-live"
}

variable "cloud-platform-iam-prod" {
  type        = string
  description = "IAM role that our API in Cloud Platform will use to connect to this role."
  default     = "arn:aws:iam::211125476974:role/cloud-platform-irsa-7a81f92a48491ef0-live"
}

module "laa_front_end_assumable_role" {
  ...
  role_name = "laa_read_ccds_data_${local.environment_shorthand}"
