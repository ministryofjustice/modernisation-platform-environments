resource "aws_instance" "ec2_webgate" {
  ...
  cpu_options {
    core_count       = local.application_data.accounts[local.environment].ec2_oracle_instance_cores_webgate
    threads_per_core = local.application_data.accounts[local.environment].ec2_oracle_instance_threads_webgate
  }
  ...
  root_block_device {
    ...
    tags = merge(local.tags,
      { Name = lower(format("%s-%s-%s", local.application_data.accounts[local.environment].instance_role_webgate, count.index + 1, "root")) },
  ...
  ebs_block_device {
    ...
    tags = merge(local.tags,
      { Name = lower(format("%s-%s-%s", local.application_data.accounts[local.environment].instance_role_webgate, count.index + 1, "swap")) },
      { device-name = "/dev/sdb" }
    )
  }
  # temp
  ebs_block_device {
    ...
    kms_key_id  = data.aws_kms_key.ebs_shared.key_id
    tags = merge(local.tags,
      { Name = lower(format("%s-%s-%s", local.application_data.accounts[local.environment].instance_role_webgate, count.index + 1, "temp")) },
      { device-name = "/dev/sdc" }
    )
  }
  # home
  ebs_block_device {
    ...
    kms_key_id  = data.aws_kms_key.ebs_shared.key_id
    tags = merge(local.tags,
      { Name = lower(format("%s-%s-%s", local.application_data.accounts[local.environment].instance_role_webgate, count.index + 1, "home")) },
      { device-name = "/dev/sdd" }
    )
  }
  ...
  ebs_block_device {
    ...
    kms_key_id  = data.aws_kms_key.ebs_shared.key_id
    tags = merge(local.tags,
      { Name = lower(format("%s-%s-%s", local.application_data.accounts[local.environment].instance_role_webgate, count.index + 1, "ccms")) },
      { device-name = "/dev/sdh" }
    )
  }
  ...
  tags = merge(local.tags,
    ...
    { instance-role = local.application_data.accounts[local.environment].instance_role_webgate },
    { instance-scheduling = local.application_data.accounts[local.environment].instance-scheduling-webgate },
    { backup = "true" }
  )
