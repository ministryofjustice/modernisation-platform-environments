...
resource "aws_ebs_volume" "conc_u03" {
  ...
  kms_key_id        = data.aws_kms_key.ebs_shared.key_id
  tags = merge(local.tags,
    { Name = lower(format("%s-%s", local.application_data.accounts[local.environment].instance_role_ebsdb, "u03")) },
    { device-name = "/dev/sdj" }
  )
}
...
resource "aws_ebs_volume" "conc_stage" {
  ...
  kms_key_id        = data.aws_kms_key.ebs_shared.key_id
  tags = merge(local.tags,
    { Name = lower(format("%s-%s", local.application_data.accounts[local.environment].instance_role_ebsdb, "stage")) },
    { device-name = "/dev/sdl" }
  )
}
