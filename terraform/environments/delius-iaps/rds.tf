...
resource "aws_db_instance" "iaps_test" {
  engine         = "oracle-ee"
  engine_version = "19"
  license_model  = "bring-your-own-license"
  instance_class = local.application_data.accounts[local.environment].db_instance_class
  db_name        = "IAPS"
  identifier     = "iaps-test"

  username                    = local.application_data.accounts[local.environment].db_user
  manage_master_user_password = true
  snapshot_identifier         = length(data.aws_ssm_parameter.iaps_snapshot_data_refresh_id_test.value) > 0 ? data.aws_ssm_parameter.iaps_snapshot_data_refresh_id_test.value : null
  db_subnet_group_name        = aws_db_subnet_group.iaps.id
  vpc_security_group_ids      = [aws_security_group.iaps_db.id]
