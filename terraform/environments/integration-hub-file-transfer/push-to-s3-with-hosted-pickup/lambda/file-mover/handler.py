...
def mover_client(role_arn, region):
    response = sts.assume_role(
        RoleArn=role_arn,
        RoleSessionName="hosted-pickup-mover",
        DurationSeconds=900,
    )
...
file_mover = FileMover(
    secrets=secrets,
    events=events,
    mover_client_factory=mover_client,
    authorised_destination_map=json.loads(os.environ["AUTHORISED_DESTINATION_MAP"]),
    mover_role_map=json.loads(os.environ["MOVER_ROLE_MAP"]),
...
@idempotent_function(
    ...
    key_prefix="managed-file-transfer/push-to-s3-with-hosted-pickup",
)
