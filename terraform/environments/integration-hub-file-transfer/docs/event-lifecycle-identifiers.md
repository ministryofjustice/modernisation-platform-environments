...
The adapter uses the native S3 EventBridge event `id` as both `fileId` and `correlationId`. This means both values are stable UUIDs, generated by AWS, that you can look up in Lambda logs if something goes wrong before the canonical event is published.
...
There is an important boundary here: Powertools only deduplicates retries of the *same native event*. If S3 emits two separate `Object Created` notifications for the same object (which can happen in rare cases), each notification has a different native event `id`. The adapter will treat them as independent ingress events and publish two `FileReceived.v1` events. This is an at-least-once guarantee, not exactly-once.
...
2. The file-received adapter publishes `FileReceived.v1`. EventBridge assigns this event `id = eb-1`. The event carries `fileId = native-1`, `correlationId = native-1`, and no `causationId`.
3. The scanning component picks up `FileReceived.v1` and stages the file to `processing/finance/april-payroll.csv` (a new S3 key and version). It publishes `FileStagedForScanning.v1` with `id = eb-2`, `fileId = native-1`, `correlationId = native-1`, and `causationId = eb-1`.
...
**When you see two `FileReceived.v1` events for the same object:** check whether their `fileId` values differ. If they do, S3 emitted two distinct native notifications. Both are legitimate ingress events from the adapter's perspective. Check whether the object was uploaded twice or whether S3 retried the notification with a new event ID.
