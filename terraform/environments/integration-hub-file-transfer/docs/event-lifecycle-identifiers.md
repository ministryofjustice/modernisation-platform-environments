...
The GuardDuty adapter records the native result as `scanResultStatus` and independently records the exact object's `GuardDutyMalwareScanStatus` tag as `tagStatus`. If that tag is absent or differs from the native result, `tagStatus` is `MISMATCH`. Routing uses `tagStatus`: `NO_THREATS_FOUND` routes to `clean`, `THREATS_FOUND` routes to `quarantine`, and `UNSUPPORTED`, `ACCESS_DENIED`, `FAILED`, or `MISMATCH` route to `investigation`. The workflow copies the exact processing version, preserving its metadata and tags before deleting that exact source version.
