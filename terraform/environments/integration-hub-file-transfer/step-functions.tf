...
module "step_function_filescanresultrecorded_workflow" {
  ...
  name = "${local.environment}-incoming-to-filescanresult-recorded-workflow"
  type = "STANDARD"

  definition = templatefile("${path.module}/step-functions/filescanresult-recorded-workflow.asl.json", {
    ...
  })
