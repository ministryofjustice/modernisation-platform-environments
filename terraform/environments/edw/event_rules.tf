...
resource "aws_cloudwatch_event_target" "create_db_snapshots" {
  ...
  input = jsonencode({ "appname" : "${local.application_data.accounts[local.environment].database_ec2_name}" })
}

resource "aws_cloudwatch_event_rule" "delete_db_snapshots" {
  name                = "${local.application_name}-${local.environment}-delete-db-snapshots"
