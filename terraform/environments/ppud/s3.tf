  ...
  policy = jsonencode({
    ...
    "Statement" : [
      ...
      },
      {
        ...
        "Principal" : {
          "AWS" : "arn:aws:iam:${data.aws_ssm_parameter.elb-account-eu-west-2[0].value}:root" # This ID is the elb-account-id for eu-west-2 obtained from https://docs.aws.amazon.com/elasticloadbalancing/latest/application/enable-access-logging.html
        }
      }
    ]
  })
