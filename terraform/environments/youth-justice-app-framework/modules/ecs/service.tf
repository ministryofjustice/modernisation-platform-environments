...
module "ecs_service" {
  ...
  load_balancer = each.value.internal_only ? {
    service = {
      target_group_arn = each.value.load_balancer_target_group_arn != null ? each.value.load_balancer_target_group_arn : var.list_of_target_group_arns["${each.key}-target-group-1"]
