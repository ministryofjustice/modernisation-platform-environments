...
resource "aws_cloudfront_response_headers_policy" "strict_transport_security" {
  ...
  security_headers_config {
    ...
    content_security_policy {
      override = true
      content_security_policy = "default-src 'none'; script-src 'self' https://public.tableau.com; style-src 'self' 'unsafe-inline'; img-src 'self' data:; font-src 'self'; connect-src 'self' https://www.gov.uk; object-src 'none'; frame-ancestors 'none';"
    }
  }
}
