...
--CREATE EXTERNAL SCHEMA yjb_case_reporting_redshift FROM POSTGRES DATABASE 'yjb_case_reporting_load_redshift' SCHEMA 'redshift' URI '${postgres_uri}' IAM_ROLE '${iam_role}' SECRET_ARN '${secret_arn}';

DROP SCHEMA IF EXISTS yjaf_auth;
CREATE EXTERNAL SCHEMA yjaf_auth FROM POSTGRES DATABASE 'yjaf' SCHEMA 'auth' URI '${postgres_uri}' IAM_ROLE '${iam_role}' SECRET_ARN '${secret_arn}';
