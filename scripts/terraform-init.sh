...
state_kms_key_id() {
  ...
  local account_id="${TERRAFORM_STATE_ACCOUNT_ID:-${BACKEND_NUMBER:-${MODERNISATION_PLATFORM_ACCOUNT_ID:-${MODERNISATION_PLATFORM_ACCOUNT_NUMBER:-}}}}"
  ...
  if [ -z "${account_id}" ] || ! [[ "${account_id}" =~ ^[0-9]{12}$ ]]; then
    ...
    echo "Set TERRAFORM_STATE_KMS_KEY_ID, TERRAFORM_STATE_ACCOUNT_ID, BACKEND_NUMBER, MODERNISATION_PLATFORM_ACCOUNT_ID, MODERNISATION_PLATFORM_ACCOUNT_NUMBER, or ENVIRONMENT_MANAGEMENT." >&2
    exit 1
  fi
