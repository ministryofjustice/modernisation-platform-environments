...
get_code_csv() {
  ...
  # join returns exit code 1 when there are no matching keys; tolerate that
...
get_images_to_delete_csv() {
  ...
  account=$(get_account_images_csv "${months:-}" "$include_backup" | clean_sort || true)
...
check_aws_error() {
  if [[ -s "$aws_error_log" ]] && grep -q 'Error' "$aws_error_log"; then
